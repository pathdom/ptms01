document.addEventListener("DOMContentLoaded", () => {
  ...
  const showToast = (message, type = "info", duration = 4000) => {
    ...
    if (type === "success") {
      ...
      iconSvg =
        '<svg viewBox="0 0 24 24"><path d="M12,2C5.9,2 1,6.9 1,13C1,19.1 5.9,24 12,24C18.1,24 23,19.1 23,13C23,6.9 18.1,2 12,2M13,18H11V16H13V18M13,14H11V8H13V14Z"/></svg>';
    } else if (type === "warning") {
      iconSvg =
        '<svg viewBox="0 0 24 24"><path d="M12,2L1,21H23L12,2M12,6L19.53,19H4.47L12,6M11,10V14H13V10H11M11,16V18H13V16H11Z"/></svg>';
    } else {
      iconSvg =
        '<svg viewBox="0 0 24 24"><path d="M11,9H13V7H11V9M12,20C7.59,20 4,16.41 4,12C4,7.59 7.59,4 12,4C16.41,4 20,7.59 20,12C20,16.41 16.41,20 12,20M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M11,17H13V11H11V17Z"/></svg>';
    }
  ...
  const rebuildChatThreads = () => {
    ...
    const globalMessages = allLoadedMessages.filter(
      (m) => m.threadId === "group-global" && !deletedIds.includes(m.id),
    );
    const globalThread = {
      id: "group-global",
    ...
    dmUids.forEach((contactUid) => {
      ...
      const threadMessages = allLoadedMessages.filter(
        (m) => m.threadId === threadId && !deletedIds.includes(m.id),
      );

      const initials = contactUser.name
        .split(" ")
        .map((n) => n[0])
        .join("")
        .substring(0, 2)
        .toUpperCase();
      const displayRole =
        contactUser.role === "admin" ? "Quản trị viên" : "Nhân viên";
  ...
  const renderThreadList = () => {
    ...
    const otherStaff = allUsersList.filter((user) => {
      ...
      const hasActiveThread = chatThreads.some(
        (t) => t.type === "dm" && t.contactUid === user.uid,
      );
    ...
    if (filteredThreads.length > 0) {
      ...
      secHeader.style.cssText =
        "font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: var(--accent); padding: 1.25rem 1rem 0.5rem 1rem; border-bottom: 1px solid var(--border); margin-bottom: 0.5rem; display: flex; align-items: center; justify-content: space-between;";
      secHeader.innerHTML = `
      ...
      filteredThreads.forEach((thread) => {
        ...
        if (!isCurrentActive) {
          const activeName =
            currentUser && currentUser.name
              ? `${currentUser.name} (${currentUser.role === "admin" ? "quản trị viên" : "nhân viên"})`
              : "";
          unreadCount = thread.messages.filter((msg) => {
            const isSentByMe =
              msg.sender === activeName ||
              (currentUser &&
                msg.senderEmail &&
                currentUser.email &&
        ...
        const lastMsg =
          thread.messages.length > 0
            ...
            : { content: "Chưa có tin nhắn", time: "" };
        const cleanSender = lastMsg.senderName
          ? `${lastMsg.senderName}: `
          : lastMsg.sender
        ...
        if (thread.type === "dm") {
          ...
          if (menuBtn && dropdown) {
            menuBtn.addEventListener("click", (e) => {
              ...
              document
                .querySelectorAll(".thread-dropdown-menu")
                .forEach((el) => {
              ...
              btnDelete.addEventListener("click", async (e) => {
                ...
                if (confirm(confirmMsg)) {
                  try {
                    ...
                    showToast(
                      `Đã xóa đoạn chat với ${thread.name} thành công!`,
                      "success",
                    );
    ...
    if (query && otherStaff.length > 0) {
      ...
      secHeader.className = "sidebar-section-header";
      secHeader.style.cssText =
        "font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: var(--accent); padding: 1.5rem 1rem 0.5rem 1rem; border-bottom: 1px solid var(--border); margin-bottom: 0.5rem; display: flex; align-items: center; justify-content: space-between;";
      secHeader.innerHTML = `
      ...
      otherStaff.forEach((user) => {
        ...
        const sentRequest = mySentRequests.find(
          (r) => r.receiverUid === user.uid && r.status === "pending",
        );
        const receivedRequest = myReceivedRequests.find(
          (r) => r.senderUid === user.uid && r.status === "pending",
        );
    ...
    filteredMessages.forEach((msg) => {
      ...
      const isSentByMe =
        ...
        (currentUser &&
          msg.sender &&
          currentUser.name &&
          msg.sender.toLowerCase().includes(currentUser.name.toLowerCase()));
      ...
      if (msg.isRecalled) {
        ...
        if (activeChatSearchQuery && msg.content) {
          const escapedQuery = activeChatSearchQuery.replace(
            /[-\/\\^$*+?.()|[\]{}]/g,
            "\\$&",
          );
        ...
        // Action Toolbar
        const canRecall =
          isSentByMe || (currentUser && currentUser.role === "admin");
      ...
      bubbleRow
        .querySelectorAll(".chat-message-file-container")
        .forEach((container) => {
  ...
  const chatMessageSearchInput = document.getElementById(
  ...
  document.addEventListener("click", (e) => {
    if (chatEmojiPicker && chatEmojiPicker.style.display === "block") {
      if (
        !chatEmojiPicker.contains(e.target) &&
        !e.target.closest("#btnToggleEmojiPicker")
      ) {
        chatEmojiPicker.style.display = "none";
      }
    }
  ...
  const _stopIdleWatch = () => {
    ...
    [
      ...
      "scroll",
      "click",
    ].forEach((evt) => document.removeEventListener(evt, _resetIdleTimer));
  };
  ...
  const setupDefaultAdmin = async () => {
    try {
      ...
      if (error.code === "auth/operation-not-allowed") {
        setTimeout(() => {
          showToast(
            "LỖI HỆ THỐNG: Vui lòng vào Firebase Console -> Authentication -> Sign-in method -> BẬT 'Email/Password' để hoạt động!",
            "error",
          );
        }, 1000);
      }
  ...
  const syncUserInfoUI = (user) => {
    ...
    const miniStudentUsersName = document.getElementById(
      "miniStudentUsersName",
    );
    const miniStudentUsersRole = document.getElementById(
      "miniStudentUsersRole",
    );
    ...
    const menuItemCreateStudentUsers = document.getElementById(
      "menuItemCreateStudentUsers",
    );
    ...
    if (user.role === "admin") {
      ...
      if (menuItemCreateStudentUsers)
        menuItemCreateStudentUsers.style.display = "flex";
      ...
      if (menuItemCreateStudentUsers)
        menuItemCreateStudentUsers.style.display = "none";
  ...
  const handlePortalLogin = async (email, password) => {
    try {
        ...
        try {
          ...
          if (!studentQuery.empty) {
            ...
            if (userQuery.empty) {
              ...
              showToast(
                "Kích hoạt tài khoản học viên thành công! Đang tự động đăng nhập...",
                "success",
              );
          ...
          console.error(
            "Failed to auto-create student user during login:",
            createErr,
          );
        }
      }
      ...
      if (
        ...
        try {
          ...
          if (userQuery.empty) {
            ...
            showToast(
              "Khởi tạo tài khoản Admin thành công! Đang tự động đăng nhập...",
              "success",
            );
            // Firebase automatically signs in the user upon creation. We sync session naturally.
            return;
          ...
          if (createErr.code === "auth/operation-not-allowed") {
            showToast(
              "LỖI HỆ THỐNG: Vui lòng vào Firebase Console -> Authentication -> Sign-in method -> BẬT 'Email/Password'!",
              "error",
            );
          } else {
      ...
      if (error.code === "auth/operation-not-allowed") {
        errorMsg =
          "LỖI HỆ THỐNG: Đăng nhập bằng Email/Mật khẩu chưa được kích hoạt trên Firebase Console của bạn. Vui lòng vào Console -> Authentication -> Sign-in method -> bật 'Email/Password'!";
  ...
  const switchPortalView = (targetViewId) => {
    ...
    if (targetElement) {
      const flexDashboards = [
        "staff-profile-dashboard",
        "staff-attendance-dashboard",
        "test-dashboard",
      ];
      targetElement.style.display = flexDashboards.includes(targetViewId)
        ? "flex"
        : "block";
    }
  ...
  const renderStaffUsersList = async () => {
    ...
    try {
      ...
      staffMembers.forEach((user) => {
        ...
        tr.querySelector(".btn-delete-staff").addEventListener(
          "click",
          async () => {
            if (
              confirm(
                `Bạn có chắc chắn muốn xóa tài khoản nhân viên ${user.name} (${user.email})?`,
              )
            ) {
              try {
                await db.collection("users").doc(user.uid).delete();
                showToast(
                  `Đã xóa tài khoản nhân viên ${user.name} thành công!`,
                  "warning",
                );
                renderStaffUsersList();
              } catch (err) {
  ...
  const handleCreateStaffUser = async (email, password, name) => {
    const secondaryAppName =
      "secondary_" + Math.random().toString(36).substring(7);
    ...
    try {
      ...
      showToast(
        `Đã tạo tài khoản nhân viên cho ${name} thành công!`,
        "success",
      );
  ...
  const renderStudentUsersList = async () => {
    ...
    try {
      ...
      studentsList.forEach((student) => {
        ...
        if (userAccount) {
          tr.querySelector(".btn-delete-student-user").addEventListener(
            "click",
            async () => {
              if (
                confirm(
                  `Bạn có chắc chắn muốn xóa tài khoản học viên ${student.name} (${student.email})? Thao tác này cũng sẽ xóa hồ sơ tư vấn tương ứng.`,
                )
              ) {
                try {
                  ...
                  showToast(
                    `Đã xóa tài khoản học viên ${student.name} thành công!`,
                    "warning",
                  );
                  renderStudentUsersList();
                } catch (err) {
          ...
          tr.querySelector(".btn-delete-student-profile-only").addEventListener(
            "click",
            async () => {
              if (
                confirm(
                  `Học viên ${student.name} chưa có tài khoản portal. Bạn có chắc chắn muốn xóa hồ sơ tư vấn của học viên này?`,
                )
              ) {
                try {
                  await db.collection("students").doc(student.id).delete();
                  showToast(
                    `Đã xóa hồ sơ học viên ${student.name} thành công!`,
                    "warning",
                  );
                  renderStudentUsersList();
                } catch (err) {
  ...
  const handleCreateStudentUser = async (
    ...
    const secondaryAppName =
      "secondary_student_" + Math.random().toString(36).substring(7);
    const secondaryApp = firebase.initializeApp(
      firebaseConfig,
      secondaryAppName,
    );
  ...
  const createStudentUserForm = document.getElementById(
  ...
  if (createStudentUserForm) {
    createStudentUserForm.addEventListener("submit", (e) => {
      ...
      const newLearningMonth = document.getElementById(
        "newStudentLearningMonth",
      ).value;
  ...
  const resetStudentPasswordForm = document.getElementById(
  ...
  if (resetStudentPasswordForm) {
    resetStudentPasswordForm.addEventListener("submit", async (e) => {
      ...
      let email = document
        .getElementById("resetStudentEmail")
        .value.trim()
        .toLowerCase();
      if (email && !email.includes("@")) {
        email = email + "@aladdin.hv";
      }
      const newDefaultPassword = document.getElementById(
        "resetStudentNewPassword",
      ).value;

      if (!email || !newDefaultPassword) {
        showToast(
          "Vui lòng nhập đầy đủ email và mật khẩu mặc định mới!",
          "error",
        );
        return;
      }
      ...
      try {
        ...
        if (userSnap.empty) {
          showToast(
            "Không tìm thấy tài khoản học viên với email này!",
            "error",
          );
          return;
        }
        ...
        showToast(
          `Đã gửi email khôi phục mật khẩu và cập nhật mật khẩu mặc định mới cho học viên!`,
          "success",
        );
  ...
  const resetStaffPasswordForm = document.getElementById(
  ...
  if (resetStaffPasswordForm) {
    resetStaffPasswordForm.addEventListener("submit", async (e) => {
      ...
      let email = document
        .getElementById("resetStaffEmail")
        .value.trim()
        .toLowerCase();
      const newDefaultPassword = document.getElementById(
      ...
      try {
        ...
        if (userSnap.empty) {
          showToast(
            "Không tìm thấy tài khoản nhân viên với email này!",
            "error",
          );
          return;
        }
        ...
        showToast(
          `Đã gửi email khôi phục mật khẩu và cập nhật mật khẩu mặc định mới cho nhân viên!`,
          "success",
        );
  ...
  const btnTriggerImageUpload = document.getElementById(
    "btnTriggerImageUpload",
  );
  ...
  const recallMessage = async (messageId) => {
    if (
      confirm("Bạn có chắc chắn muốn thu hồi tin nhắn này đối với mọi người?")
    ) {
      try {
  ...
  const deleteMessage = async (messageId, canEveryone) => {
    ...
    if (canEveryone) {
      const choice = confirm(
        "Nhấn OK để 'Xóa ở mọi người' (Xóa vĩnh viễn khỏi hệ thống), nhấn Hủy (Cancel) để 'Xóa chỉ ở phía tôi'?",
      );
      if (choice) {
        deleteForEveryone = true;
      } else {
        const confirmMe = confirm(
          "Bạn có muốn xóa tin nhắn này ở phía bạn không?",
        );
        if (!confirmMe) return;
  ...
  const forwardMessageToThread = async (msgId, targetThreadId) => {
    ...
    try {
      ...
      const payload = {
        ...
        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
        forwardedFrom:
          originalMsg.senderName || originalMsg.sender.split(" (")[0],
      };
  ...
  const updateFriendBadge = () => {
    ...
    const count = myReceivedRequests.filter(
      (r) => r.status === "pending",
    ).length;
  ...
  const subscribeToFriendRequests = () => {
    ...
    receivedRequestsSubscription = db
      .collection("friend_requests")
  ...
  const btnCloseFindFriendsModal = document.getElementById(
    "btnCloseFindFriendsModal",
  );
  ...
  const renderFriendsSearchResults = (query = "") => {
    ...
    if (!q) {
      ...
      const pendingReceived = myReceivedRequests.filter(
        (r) => r.status === "pending",
      );
      if (pendingReceived.length > 0) {
        ...
        header.style.cssText =
          "font-size: 0.8rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: var(--accent); padding: 1rem 0.5rem 0.5rem 0.5rem; margin-top: 0.5rem; border-bottom: 1px solid var(--border);";
        ...
        pendingReceived.forEach((req) => {
          ...
          item.className = "forward-target-item";
          item.style.cssText =
            "display: flex; align-items: center; gap: 1rem; padding: 0.75rem 0.5rem; border-radius: var(--border-radius-md); margin-top: 0.5rem; cursor: default;";
          item.innerHTML = `
          ...
          // Bind accept click
          item
            .querySelector(".btn-accept-friend")
            .addEventListener("click", async (e) => {
              ...
              try {
                ...
                showToast(
                  `Đã chấp nhận lời mời kết bạn từ ${req.senderName}!`,
                  "success",
                );
              } catch (err) {
          ...
          // Bind decline click
          item
            .querySelector(".btn-decline-friend")
            .addEventListener("click", async (e) => {
              const btn = e.target;
              btn.disabled = true;
              btn.textContent = "Đang xử lý...";
              try {
                await db.collection("friend_requests").doc(req.id).delete();
                showToast(
                  `Đã từ chối lời mời kết bạn từ ${req.senderName}.`,
                  "info",
                );
              } catch (err) {
      ...
      if (pendingSent.length > 0) {
        ...
        header.className = "sidebar-section-header";
        header.style.cssText =
          "font-size: 0.8rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: var(--accent); padding: 1rem 0.5rem 0.5rem 0.5rem; margin-top: 1rem; border-bottom: 1px solid var(--border);";
        header.textContent = "Yêu cầu kết bạn đã gửi";
        list.appendChild(header);

        pendingSent.forEach((req) => {
          const initials = req.receiverName
            .split(" ")
            .map((n) => n[0])
            .join("")
            .substring(0, 2)
          ...
          item
            .querySelector(".btn-cancel-request")
            .addEventListener("click", async (e) => {
              const btn = e.target;
              btn.disabled = true;
              ...
              try {
                await db.collection("friend_requests").doc(req.id).delete();
                showToast(
                  `Đã hủy yêu cầu kết bạn gửi tới ${req.receiverName}.`,
                  "info",
                );
              } catch (err) {
  ...
  const btnDownloadFromContext = document.getElementById(
    "btnDownloadFromContext",
  );
  if (btnDownloadFromContext) {
    btnDownloadFromContext.addEventListener("click", () => {
      ...
      try {
        ...
        console.error(
          "Blob URL download failed, falling back to direct click",
          err,
        );
        const a = document.createElement("a");
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      ...
      !e.target.closest(".thread-dropdown-menu")
    ) {
  ...
  const _loadStaffMap = async () => {
    ...
    try {
      ...
      const cached =
        typeof hrmStaffCache !== "undefined" && hrmStaffCache.length
          ? hrmStaffCache
          : (await db.collection("hrm_staff").get()).docs.map((d) => ({
              id: d.id,
              ...d.data(),
            }));
      _staffNameMap = {};
      ...
      cached.forEach((s) => {
        ...
        _staffNameMap[s.name.toLowerCase().trim()] = {
          name: s.name,
          department: dept || "--",
          email: s.email || "",
        };
      ...
      _staffDepts = _BASE_DEPTS.concat(
        [..._deptSet].filter((d) => !_BASE_DEPTS.includes(d)),
      );
    } catch (e) {
  ...
  const renderStudentsTable = (filteredList) => {
    ...
    const globalStudentIndexMap = new Map(
      students.map((s, i) => [s.id, 10001 + i]),
    );
    ...
    filteredList.forEach((rawStudent, fIdx) => {
      ...
      else if (student.status === "Đang làm hồ sơ")
      ...
      if (student.flightDate) {
        const fd2 = student.flightDate.toDate
          ? student.flightDate.toDate()
          : new Date(student.flightDate);
        if (!isNaN(fd2)) {
          const fy = fd2.getFullYear(),
            fm = String(fd2.getMonth() + 1).padStart(2, "0"),
            fdd = String(fd2.getDate()).padStart(2, "0");
          fVal = fy + "-" + fm + "-" + fdd;
          fDisplay = fdd + "/" + fm + "/" + fy;
        }
      }
      ...
      const room = staffMatch
        ? staffMatch.department
        : student.room || student.classroom || "--";

      tr.innerHTML =
        '<td class="stw-code"><span class="stw-code-val">' +
        (student.code || displayCode) +
        "</span></td>" +
        '<td class="stw-name"><div class="stw-name-main">' +
        (student.name || "--") +
        "</div></td>" +
        '<td class="stw-email"><a class="stw-link" href="mailto:' +
        (student.email || "") +
        '">' +
        (student.email || "--") +
        "</a></td>" +
        ...
        hometown +
        "</td>" +
        '<td class="stw-country"><span class="stw-country-val">' +
        (student.country || "--") +
        "</span></td>" +
        '<td class="stw-status">' +
        '<select class="stw-status-select ' +
        badgeClass +
        '" data-id="' +
        student.id +
        '">' +
        ...
        ">Đang học</option>" +
        '<option value="Chờ phỏng vấn"' +
        (student.status === "Chờ phỏng vấn" ? " selected" : "") +
        ">Chờ phỏng vấn</option>" +
        '<option value="Đã trúng tuyển"' +
        (student.status === "Đã trúng tuyển" ? " selected" : "") +
        ">Đã trúng tuyển</option>" +
        '<option value="Đang làm hồ sơ"' +
        (student.status === "Đang làm hồ sơ" ? " selected" : "") +
        ">Đang làm hồ sơ</option>" +
        '<option value="Đã xuất cảnh"' +
        (student.status === "Đã xuất cảnh" ? " selected" : "") +
        ">Đã xuất cảnh</option>" +
        '<option value="Chờ xử lý"' +
        (student.status === "Chờ xử lý" ? " selected" : "") +
        ">Chờ xử lý</option>" +
        "</select>" +
        "</td>" +
        '<td class="stw-roadmap"><span class="stw-roadmap-tag">' +
        roadmapLabel +
        "</span></td>" +
        '<td class="stw-enroll">' +
        enrollDateStr +
        ...
        (() => {
          const opts = ['<option value="">-- Chọn phòng --</option>']
            ...
            .join("");
          return (
            '<td class="stw-room"><select class="stw-inline-select stw-room-select" data-id="' +
            student.id +
            '">' +
            opts +
            "</select></td>"
        ...
        (() => {
          ...
          const opts = ['<option value="">-- Chọn nhân viên --</option>']
            ...
            .join("");
          return (
            '<td class="stw-src"><select class="stw-inline-select stw-src-select" data-id="' +
            student.id +
            '">' +
            opts +
            "</select></td>"
        ...
        fVal +
        '" title="' +
        (fVal ? "Ngày bay: " + fDisplay : "Chưa có lịch bay") +
        '" />' +
        "</td>" +
        '<td class="stw-paid">' +
        '<input type="text" class="stw-money-input stw-paid-input" data-id="' +
        student.id +
        '" data-field="paidAmount"' +
        ' value="' +
        (student.paidAmount ? fmtMoneyInput(student.paidAmount) : "") +
        '"' +
        ' placeholder="0" inputmode="numeric" autocomplete="off" />' +
        "</td>" +
        '<td class="stw-total">' +
        '<input type="text" class="stw-money-input stw-total-input" data-id="' +
        student.id +
        '" data-field="totalAmount"' +
        ' value="' +
        (student.totalAmount ? fmtMoneyInput(student.totalAmount) : "") +
        '"' +
        ' placeholder="0" inputmode="numeric" autocomplete="off" />' +
        "</td>" +
        '<td class="stw-action"><div class="stw-actions">' +
        '<button class="action-icon-btn btn-view-student" data-id="' +
        student.id +
        '" title="Chi tiết" style="color:#6366F1;background:#EEF2FF;">' +
        ...
        "</button>" +
        '<button class="action-icon-btn btn-edit-student" data-id="' +
        student.id +
        '" title="Sửa" style="color:var(--text-main);background:var(--bg-secondary,#F7F4EF);">' +
        ...
        "</button>" +
        '<button class="action-icon-btn btn-delete-student" data-id="' +
        student.id +
        '" title="Xóa" style="color:#EF4444;background:#FEF2F2;">' +
        '<svg viewBox="0 0 24 24" style="width:14px;height:14px;fill:currentColor;"><path d="M19,4H15.5L14.5,3H9.5L8.5,4H5V6H19M6,19A2,2 0 0,0 8,21H16A2,2 0 0,0 18,19V7H6V19Z"/></svg>' +
        "</button>" +
        "</div></td>";
      ...
      tr.querySelector(".stw-status-select").addEventListener(
        "change",
        async function () {
          ...
          else if (newStatus === "Chờ phỏng vấn")
            this.classList.add("badge-waiting");
          else if (newStatus === "Đã trúng tuyển")
            this.classList.add("badge-selected");
          else if (newStatus === "Đang làm hồ sơ")
            this.classList.add("badge-processing");
          else if (newStatus === "Đã xuất cảnh")
            this.classList.add("badge-selected");
          try {
            await db
              .collection("students")
      ...
      tr.querySelector(".stw-src-select").addEventListener(
        "change",
        async function () {
          const sid = this.dataset.id;
          ...
          if (roomSel && newDept) {
            const exists = [...roomSel.options].some(
              (o) => o.value === newDept,
            );
            if (!exists) {
          ...
          try {
            ...
            showToast(
              "Đã cập nhật nguồn" + (newDept ? " & phòng: " + newDept : ""),
              "success",
            );
          } catch (err) {
      ...
      tr.querySelector(".stw-room-select").addEventListener(
        "change",
        async function () {
          const sid = this.dataset.id;
      ...
      tr.querySelector(".flight-date-input").addEventListener(
        "change",
        async function () {
          ...
          try {
            ...
            const daysLeft = fd3
              ? Math.round((fd3 - new Date()) / 86400000)
              : null;
            showToast(
              dateStr
                ? "Đã lưu lịch bay " +
                    sname +
                    " · " +
                    (daysLeft >= 0 ? "còn " + daysLeft + " ngày" : "đã qua") +
                    " · Thông báo sẽ gửi trước 7 ngày và 3 ngày!"
  ...
  const applyStudentFiltersAndRender = () => {
    ...
    const filtered = students.filter((student) => {
      ...
      const countryMatch =
        countryVal === "All" || student.country === countryVal;
    ...
    if (paginatedStart)
      paginatedStart.textContent = totalItems === 0 ? 0 : startIndex + 1;
    if (paginatedEnd) paginatedEnd.textContent = endIndex;
    if (paginatedTotal) paginatedTotal.textContent = totalItems;
    if (currentPageLabel)
      currentPageLabel.textContent = `Trang ${currentPage} / ${totalPages}`;
      ...
      newBtnNext.addEventListener("click", () => {
        ...
        const filteredCount = students.filter((student) => {
          ...
          const countryMatch =
            countryVal === "All" || student.country === countryVal;
          const statusMatch =
            statusVal === "All" || student.status === statusVal;
          return textMatch && countryMatch && statusMatch;
        }).length;
  ...
  const btnOpenAddStudentModal = document.getElementById(
    "btnOpenAddStudentModal",
  );
  ...
  const populateAdvisorSelect = async (selectedName = "") => {
    ...
    sel.innerHTML =
      ...
      staffList
        .map(
          (s) =>
            `<option value="${s.name}"${s.name === selectedName ? " selected" : ""}>${s.name}${s.position ? " · " + s.position : ""}</option>`,
        )
        .join("");
  ...
  if (btnOpenAddStudentModal && studentModal) {
    btnOpenAddStudentModal.addEventListener("click", async () => {
      ...
      document
        .getElementById("studentLearningMonth")
  ...
  const btnCloseStudentDetailModal = document.getElementById(
    "btnCloseStudentDetailModal",
  );
  if (btnCloseStudentDetailModal)
    btnCloseStudentDetailModal.addEventListener(
      "click",
      closeStudentDetailModal,
    );

  const btnCloseDetailModalAction = document.getElementById(
    "btnCloseDetailModalAction",
  );
  if (btnCloseDetailModalAction)
    btnCloseDetailModalAction.addEventListener(
      "click",
      closeStudentDetailModal,
    );
  ...
  const openStudentDetailModal = (student) => {
    ...
    const initials = (student.name || "HV")
      .split(" ")
      .map((n) => n[0])
      .join("")
      .substring(0, 2)
    ...
    if (statusEl) {
      ...
      const sc =
        student.status === "Đang học"
          ? "active-badge"
          : student.status === "Chờ phỏng vấn"
            ? "pending-badge"
            : student.status === "Đã xuất cảnh"
              ? "completed-badge"
              : "inactive-badge";
    ...
    if (student.flightDate) {
      const fd = student.flightDate.toDate
        ...
        : new Date(student.flightDate);
      if (flightEl)
        flightEl.textContent = fd.toLocaleDateString("vi-VN", {
          day: "2-digit",
          month: "2-digit",
          year: "numeric",
        });
      if (countdownEl) {
        ...
        countdownEl.textContent =
          diff > 0
            ...
            : diff === 0
              ? "Hôm nay xuất cảnh!"
              : "Đã xuất cảnh " + Math.abs(diff) + " ngày trước";
        countdownEl.style.background =
          diff <= 7 && diff >= 0 ? "rgba(239,68,68,0.1)" : "";
    ...
    const fmtTu = (n) =>
      n > 0 ? Number(n).toLocaleString("vi-VN") + " đ" : "--";
    ...
    if (tuStatusEl) {
      if (!tuitionTotal) {
        tuStatusEl.textContent = "Chưa cập nhật";
        tuStatusEl.style.background = "var(--color-border,#E8E5DF)";
        tuStatusEl.style.color = "var(--color-text-muted,#6B6A67)";
      } else if (tuitionRemain <= 0) {
        tuStatusEl.textContent = "Đã đóng đủ";
        tuStatusEl.style.background = "rgba(16,185,129,0.1)";
        tuStatusEl.style.color = "#10B981";
      } else {
        tuStatusEl.textContent = "Còn nợ";
        tuStatusEl.style.background = "rgba(239,68,68,0.1)";
        tuStatusEl.style.color = "#EF4444";
      }
    }
    ...
    if (barEl)
      barEl.style.width =
        tuitionTotal > 0
          ? Math.min(100, Math.round((tuitionPaid / tuitionTotal) * 100)) + "%"
          : "0%";

    const ROADMAP_STEPS_VI = [
      {
        month: "Tháng 1",
        label: "Xây dựng nền tảng",
        sub: "Nhập môn ngôn ngữ & văn hóa",
      },
      {
        month: "Tháng 2",
        label: "Phát triển phản xạ",
        sub: "Kỹ năng nghe – nói cơ bản",
      },
      {
        month: "Tháng 3",
        label: "Làm quen học thuật",
        sub: "Ngữ pháp & từ vựng học thuật",
      },
      {
        month: "Tháng 4",
        label: "Tăng tốc học thuật",
        sub: "Đọc hiểu & viết luận",
      },
      {
        month: "Tháng 5",
        label: "Luyện đề chuyên sâu",
        sub: "Ôn thi & mô phỏng phỏng vấn",
      },
      {
        month: "Tháng 6",
        label: "Tổng ôn & mô phỏng",
        sub: "Chuẩn bị hồ sơ & xuất cảnh",
      },
    ];
    ...
    if (roadmapEl) {
      roadmapEl.innerHTML = ROADMAP_STEPS_VI.map(function (s, i) {
        ...
        const statusIcon = done
          ? '<svg viewBox="0 0 24 24"><path d="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z"/></svg>'
          : active
            ? '<span class="stp-active-dot"></span>'
            : "";
        return (
          '<div class="stp-roadmap-step ' +
          cardCls +
          '">' +
          '<div class="stp-step-dot ' +
          dotCls +
          '" data-month="' +
          s.month +
          '">' +
          statusIcon +
          "</div>" +
          '<div class="stp-step-body"><div class="stp-step-label">' +
          s.label +
          "</div>" +
          '<span class="stp-step-tag ' +
          tagCls +
          '">' +
          tagTxt +
          "</span></div></div>"
    ...
    const applyTag = function (id, txt) {
      ...
      el.style.background =
        txt === "Hoàn thành"
          ...
          : txt === "Đang học"
            ? "rgba(168,139,88,0.1)"
            : "var(--color-border,#E8E5DF)";
      el.style.color =
        txt === "Hoàn thành"
          ? "#10B981"
          : txt === "Đang học"
            ? "var(--color-accent,#A88B58)"
            : "var(--color-text-muted,#6B6A67)";
    };
    applyTag("adsdSem1", sem1Status);
    applyTag("adsdSem2", sem2Status);
    applyTag("adsdSem3", sem3Status);
    const semNames = {
      1: "KÌ I: Nhập môn & Phản xạ",
      2: "KÌ I: Nhập môn & Phản xạ",
      3: "KÌ II: Ngữ pháp & Học thuật",
      4: "KÌ II: Ngữ pháp & Học thuật",
      5: "KÌ III: Luyện đề & Phỏng vấn",
      6: "KÌ III: Luyện đề & Phỏng vấn",
      7: "Hoàn thành lộ trình",
    };
    ...
    if (student.id) {
      db.collection("student_profiles")
        ...
        .then((snap) => {
          ...
          const fmtYr = (name, year, job) =>
            [name, year ? `(${year})` : "", job].filter(Boolean).join(" · ") ||
            "--";
          const fmtEdu = (name, from, to) =>
            name
              ? `${name}${from || to ? ` (${from || ""}–${to || ""})` : ""}`
              : "--";
          sd("adsdDCccd", d.cccd);
          sd("adsdDCccdDate", d.cccdDate);
          sd("adsdDGender", d.gender);
          sd("adsdDDob", d.dob);
          sd("adsdDReligion", d.religion);
          sd("adsdDEthnicity", d.ethnicity);
          sd("adsdDMarital", d.marital);
          sd("adsdDPhone", d.phone);
          sd("adsdDPhoneRelative", d.phoneRelative);
          sd("adsdDPermanentAddress", d.permanentAddress);
          sd("adsdDTempAddress", d.tempAddress);
          sd(
            "adsdDSchoolPrimary",
            fmtEdu(d.schoolPrimary, d.schoolPrimaryFrom, d.schoolPrimaryTo),
          );
          sd(
            "adsdDSchoolMiddle",
            fmtEdu(d.schoolMiddle, d.schoolMiddleFrom, d.schoolMiddleTo),
          );
          sd(
            "adsdDSchoolHigh",
            fmtEdu(d.schoolHigh, d.schoolHighFrom, d.schoolHighTo),
          );
          sd(
            "adsdDSchoolUni",
            fmtEdu(d.schoolUni, d.schoolUniFrom, d.schoolUniTo),
          );
          sd("adsdDFather", fmtYr(d.fatherName, d.fatherYear, d.fatherJob));
          sd("adsdDMother", fmtYr(d.motherName, d.motherYear, d.motherJob));
          sd(
            "adsdDSiblingOlder",
            fmtYr(d.siblingOlderName, d.siblingOlderYear, d.siblingOlderJob),
          );
          sd(
            "adsdDSiblingYounger",
            fmtYr(
              d.siblingYoungerName,
              d.siblingYoungerYear,
              d.siblingYoungerJob,
            ),
          );
          sd(
            "adsdDOtherMember",
            fmtYr(d.otherMemberName, d.otherMemberYear, d.otherMemberJob),
          );
          sd("adsdDStrengths", d.strengths);
          sd("adsdDWeaknesses", d.weaknesses);
          sd("adsdDReason", d.reason);
          sd("adsdDHobbies", d.hobbies);
    ...
    if (dtabContainer && !dtabContainer.dataset.bound) {
      ...
      dtabContainer.addEventListener("click", (e) => {
        ...
        if (!btn) return;
        overlay
          .querySelectorAll(".adsd-dtab")
          .forEach((t) => t.classList.remove("active"));
        overlay
          .querySelectorAll(".adsd-dpanel")
    ...
    requestAnimationFrame(() => {
      semBarData.forEach(({ barId, tagId }) => {
        ...
        bar.style.width =
          txt === "Hoàn thành" ? "100%" : txt === "Đang học" ? "50%" : "0%";
  ...
  const openEditStudentModal = async (student) => {
    ...
    if (hometownEl)
      hometownEl.value = student.hometown || student.address || "";
    ...
    if (paidEl)
      paidEl.value = student.paidAmount
        ? fmtMoneyInput(student.paidAmount)
        : "";
    if (totalEl)
      totalEl.value = student.totalAmount
        ? fmtMoneyInput(student.totalAmount)
        : "";
  ...
  const handleDeleteStudent = async (student) => {
    if (
      confirm(
        `Bạn có chắc chắn muốn xóa hồ sơ học viên ${student.name} (${student.code})?`,
      )
    ) {
      try {
        await db.collection("students").doc(student.id).delete();
  ...
  if (studentForm) {
    studentForm.addEventListener("submit", async (e) => {
      ...
      const learningMonth = document.getElementById(
        "studentLearningMonth",
      ).value;
      const notes = document.getElementById("studentNotes").value.trim();
      const advisor = document.getElementById("studentAdvisor")?.value || "";
      const hometown =
        document.getElementById("studentHometown")?.value.trim() || "";
      const room = document.getElementById("studentRoom")?.value || "";
      const formMode =
        document.getElementById("studentFormMode")?.value || "student";
      const paidAmount = parseMoneyInput(
        document.getElementById("studentPaidAmount")?.value || "",
      );
      const totalAmount = parseMoneyInput(
        document.getElementById("studentTotalAmount")?.value || "",
      );
      const enrollDate =
        document.getElementById("studentEnrollDate")?.value || null;
      const flightDate =
        document.getElementById("studentFlightDate")?.value || null;

      if (!name || !code || !email || !phone) {
        showToast(
          "Vui lòng nhập đầy đủ các trường thông tin có dấu *!",
          "error",
        );
        return;
      }
      ...
      const createStudentUserAccount = async (email, name) => {
        ...
        try {
          ...
          console.log(
            "Successfully created Auth & users collection entry for new student:",
            email,
          );
        } catch (authErr) {
          console.warn(
            "Failed or skipped creating Auth user:",
            authErr.message,
          );
          ...
          try {
            ...
            console.error(
              "Error updating users collection for existing email:",
              dbErr,
            );
          }
        } finally {
      ...
      try {
        if (editId) {
          ...
          showToast(
            `Đã cập nhật hồ sơ học viên ${name} thành công!`,
            "success",
          );
        } else {
          // Add
          ...
          try {
            ...
            console.error(
              "Failed to auto-create account for new student:",
              accErr,
            );
          }

          showToast(
            `Đã thêm mới hồ sơ học viên ${name} thành công!`,
            "success",
          );
        }
        closeStudentModal();
  ...
  const handleExportExcel = () => {
    ...
    if (filtered.length === 0) {
      showToast(
        "Không có bản ghi nào phù hợp với bộ lọc hiện tại để xuất!",
        "warning",
      );
      return;
    }
    ...
    let csvContent =
      "MÃ HỌC VIÊN,HỌ VÀ TÊN,EMAIL,SỐ ĐIỆN THOẠI,QUỐC GIA ĐẾN,TRẠNG THÁI HỒ SƠ,LỘ TRÌNH HỌC TẬP,GHI CHÚ\n";

    filtered.forEach((s) => {
      const notesClean = (s.notes || "")
        .replace(/"/g, '""')
        .replace(/\n/g, " ");
    ...
    link.setAttribute(
      "download",
      `Danh_Sach_Hoc_Vien_ThinkEdu_${new Date().toLocaleDateString("vi-VN").replace(/\//g, "-")}.csv`,
    );
    ...
    showToast(
      `Đã xuất thành công ${filtered.length} hồ sơ học viên ra Excel CSV!`,
      "success",
    );
  };
    ...
    reader.onload = async function (evt) {
      try {
        ...
        for (const row of rows) {
          ...
          const name =
            row["Họ Tên"] || row["Name"] || row["Họ tên"] || row["ho_ten"];
          ...
          let typeVal =
            ...
            row["Type"] ||
            row["loai"];
          const indexVal = parseInt(
            row["Số (Tuần/Tháng số mấy)"] ||
              row["Số"] ||
              row["Index"] ||
              row["so"],
          );
        ...
        showToast(
          `Đã nhập điểm thành công! Khớp ${successCount} bản ghi. Bỏ qua ${errorCount} dòng không hợp lệ.`,
          "success",
        );

        const activeModalStudentEmail =
          document.getElementById("detailStudentEmail")?.textContent;
        if (activeModalStudentEmail) {
          const studentQuery = await db
            .collection("students")
            .where("email", "==", activeModalStudentEmail)
            .get();
  ...
  const btnDownloadTemplateExcel = document.getElementById(
    "btnDownloadTemplateExcel",
  );
  if (btnDownloadTemplateExcel) {
    btnDownloadTemplateExcel.addEventListener(
      "click",
      handleDownloadTemplateExcel,
    );
  }

  const scorecardExcelFileInput = document.getElementById(
    "scorecardExcelFileInput",
  );
  ...
  const defaultBlogs = [
    {
      ...
      summary:
        "Tìm hiểu các yêu cầu cốt lõi trong hồ sơ xin tư cách lưu trú (COE) du học Nhật. Những điểm cần đặc biệt lưu ý về chứng minh tài chính và lộ trình học tập tối ưu từ cố vấn ThinkEdu.",
    ...
    },
    {
      ...
      summary:
        "Visa thẳng giúp rút ngắn thời gian xét duyệt hồ sơ Hàn Quốc. Dưới đây là danh sách các trường đại học ưu tiên (trường 1%) và cách hoàn thiện hồ sơ để có tỉ lệ đỗ tối đa.",
    ...
    },
    {
      ...
      summary:
        "Học bổng MOE và các suất học bổng trường học Đài Loan luôn hấp dẫn. Cẩm nang tổng hợp điều kiện chuẩn bị chứng chỉ TOCFL, viết bài luận cá nhân và kỹ năng phỏng vấn xuất sắc.",
    ...
    },
    {
      ...
      summary:
        "Chia sẻ chân thực từ bạn Thảo Chi - cựu học sinh ThinkEdu đang học tập tại Tokyo về những ngày đầu làm quen với ga tàu, văn hóa bản xứ và cuộc sống tự lập đáng nhớ.",
      image: "japan_news_thumbnail.png",
    },
    {
      ...
      summary:
        "Cùng ngắm nhìn những khoảnh khắc tuyệt vời của cộng đồng học sinh ThinkEdu tham gia trại hè giao lưu văn hóa và trải nghiệm cuộc sống sinh viên đầy màu sắc tại Yonsei.",
      image: "korea_news_thumbnail.png",
    },
    {
      ...
      summary:
        "Đánh giá khách quan của du học sinh về môi trường sống an toàn, chi phí sinh hoạt cực kỳ hợp lý cùng nét văn hóa ẩm thực độc đáo tại chợ đêm Đài Bắc.",
      image: "taiwan_news_thumbnail.png",
    },
    {
      ...
      summary:
        "Hướng dẫn chi tiết về quy định làm thêm 28 giờ/tuần tại Nhật Bản. Gợi ý các công việc phổ biến lương cao và cách xin giấy phép hoạt động ngoài tư cách lưu trú.",
      image: "japan_news_thumbnail.png",
    },
    {
      ...
      summary:
        "Nhật ký hình ảnh ghi lại vẻ đẹp lãng mạn của mùa thu xứ kim chi qua ống kính du học sinh ThinkEdu. Nguồn động lực to lớn cho những bạn đang ấp ủ giấc mơ Hàn Quốc.",
      image: "korea_news_thumbnail.png",
    },
    {
      ...
      summary:
        "Phân tích tiềm năng nghề nghiệp rộng mở tại các tập đoàn công nghệ hàng đầu Đài Loan. Chính sách hỗ trợ thực tập và ở lại làm việc sau tốt nghiệp cho học sinh quốc tế.",
      image: "taiwan_news_thumbnail.png",
    },
  ];
  ...
  const renderStudentBlogsGrid = () => {
    ...
    blogs.forEach((blog) => {
      ...
      if (blog.image === "korea_news_thumbnail.png")
        fallbackUrl = "godly.website_website_pa-lais-104.png";

      const card = document.createElement("div");
      card.className = "blog-card";
      card.style.cssText =
        "background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--border-radius-md); overflow: hidden; display: flex; flex-direction: column; transition: var(--transition-smooth); box-shadow: var(--shadow-sm);";
      card.setAttribute(
        "onmouseover",
        "this.style.transform='translateY(-5px)'; this.style.boxShadow='var(--shadow-md)'",
      );
      card.setAttribute(
        "onmouseout",
  ...
  const renderAdminBlogsList = () => {
    ...
    blogs.forEach((blog) => {
      ...
      tr.querySelector(".btn-edit-blog").addEventListener("click", () =>
        setupEditBlog(blog),
      );
      tr.querySelector(".btn-delete-blog").addEventListener("click", () =>
  ...
  const setupEditBlog = (blog) => {
    ...
    if (
      [
        "japan_news_thumbnail.png",
        "korea_news_thumbnail.png",
        "taiwan_news_thumbnail.png",
      ].includes(blog.image)
    ) {
    ...
    document.getElementById("btnAdminCancelBlogEdit").style.display =
      "inline-block";
  };
  ...
  const adminBlogCustomImageGroup = document.getElementById(
    "adminBlogCustomImageGroup",
  );
  const formPreviewImg = document.getElementById("adminBlogFormImagePreview");
    ...
    adminBlogImageSelect.addEventListener("change", (e) => {
      if (e.target.value === "custom") {
        ...
        if (formPreviewImg) {
          ...
          formPreviewImg.style.display = customBlogImageBase64
            ? "block"
            : "none";
        }
      } else {
  ...
  const adminBlogImageFileInput = document.getElementById(
    "adminBlogImageFileInput",
  );
  ...
  if (adminBlogForm) {
    adminBlogForm.addEventListener("submit", async (e) => {
      ...
      const category = document
        .getElementById("adminBlogCategory")
        .value.trim()
        .toUpperCase();
      const summary = document.getElementById("adminBlogSummary").value.trim();
      const imageSelectVal = document.getElementById(
        "adminBlogImageSelect",
      ).value;
  ...
  if (profileAvatarPreview)
    profileAvatarPreview.addEventListener("click", triggerAvatarClick);
  ...
  const generateScoresForStudent = (student, type, index) => {
    ...
    const getValBetween = (min, max, offset) => {
      ...
      return parseFloat(
        (min + (currentHash % ((max - min) * 10)) / 10).toFixed(1),
      );
    };
  ...
  const initStudentScorecardModule = async (profileData) => {
    ...
    if (timeVal)
      timeVal.textContent = `Tuần ${activeWeeks} / ${studyTime.timeLabel}`;
    ...
    const attStatusLabel = document.getElementById(
  ...
  const initAdminStudentScorecardModule = async (student) => {
    ...
    const renderAdminList = (type) => {
      const container =
        ...
        document.getElementById("adminStudentScorecardList");
      if (!container) return;
      container.innerHTML = "";
      ...
      for (let i = count; i >= 1; i--) {
        ...
        const card = document.createElement("div");
        card.style.cssText =
          "background: var(--bg-primary); border: 1px solid var(--border); padding: 1rem; border-radius: var(--border-radius-sm); display: flex; flex-direction: column; gap: 0.5rem; transition: var(--transition-fast);";
        card.innerHTML = `
    ...
    if (btnWeek && btnMonth) {
      ...
      const newBtnMonth = document.getElementById(
        "btnAdminScorecardMonthToggle",
      );
      ...
      newBtnWeek.classList.toggle(
        "active",
        currentAdminScorecardType === "week",
      );
      newBtnMonth.classList.toggle(
        "active",
        currentAdminScorecardType === "month",
      );
    }
  ...
  const initStudentNewsTab = (p) => {
    const announcements = [
      {
        id: 1,
        title: "Lịch nghỉ lễ Quốc khánh 2/9 & Thi thử giữa kỳ",
        category: "Nhắc nhở",
        date: "2026-08-05",
        urgency: "urgent",
      },
      {
        id: 2,
        title:
          "Khai giảng lớp tiếng Nhật/Hàn/Đài đàm thoại phản xạ miễn phí lớp tối",
        category: "Tin mới",
        date: "2026-08-02",
        urgency: "info",
      },
      {
        id: 3,
        title:
          "Thông báo về việc hoàn thiện hồ sơ dịch thuật đợt 2 kỳ nhập học tháng 4/2027",
        category: "Hỏa tốc",
        date: "2026-07-28",
        urgency: "critical",
      },
    ...
    if (notifContainer) {
      notifContainer.innerHTML = announcements
        .map((a) => {
          const badgeColor =
            a.urgency === "critical"
              ? "rgba(239,68,68,0.1)"
    ...
    const vinhDanhList = [
      {
        name: "Vũ Thùy Chi",
        code: "TE-2026-015",
        text: "Đạt GPA xuất sắc 9.6/10 và đỗ COE du học Tokyo, Nhật Bản kỳ tháng 10/2026",
        type: "study",
      },
      {
        name: "Trần Minh Quân",
        code: "TE-2026-033",
        text: "Chinh phục thành công học bổng toàn phần Chính phủ Đài Loan MOE 2026",
        type: "scholarship",
      },
      {
        name: "Lê Hồng Nhung",
        code: "TE-2026-008",
        text: "Xuất sắc đỗ Visa thẳng Đại học Kookmin, Hàn Quốc chỉ sau 2 tuần xét duyệt",
        type: "visa",
      },
    ];
    ...
    if (hofContainer) {
      hofContainer.innerHTML = vinhDanhList
        .map((v) => {
          const icon =
            v.type === "study" ? "🎓" : v.type === "scholarship" ? "🎖️" : "✈️";
          return `
    ...
    const updateClock = () => {
      ...
      if (diff <= 0) {
        ...
        if (clockEl)
          clockEl.innerHTML = `<span style="font-size:0.9rem; color: var(--accent); font-weight: 600;">Sự kiện đang diễn ra hoặc đã kết thúc!</span>`;
        return;
      }
  ...
  const initStudentAcademicTab = (p) => {
    ...
    if (status === "Đang học") {
      ...
      currentStepIdx = 3;
      statusText =
        "Hồ sơ đã gửi đi. Đang chờ phỏng vấn / xét duyệt tư cách lưu trú (COE)";
    } else if (status === "Đã trúng tuyển") {
      currentStepIdx = 5;
      statusText =
        "Đã trúng tuyển, có COE/Thư mời. Đang tiến hành xin dán Visa";
    ...
    if (stepsContainer) {
      stepsContainer.innerHTML = steps
        .map((s, idx) => {
          ...
          const active =
            stepNum === currentStepIdx && status !== "Đã xuất cảnh";
          const dotBg = done
            ? "var(--accent)"
            : active
              ? "#FAF9F6"
          ...
          const dotBorder = done
            ? "4px solid var(--accent-light)"
            : active
              ? "5px solid var(--accent)"
              : "4px solid var(--bg-card)";
          const titleColor =
            done || active ? "var(--text-main)" : "var(--text-muted)";
      ...
      Đài: [
        {
          ...
          note: "Bản gốc đối chiếu",
        },
        {
          name: "Giấy khai sinh bản sao trích lục",
          note: "Dịch thuật công chứng Văn phòng Đài Bắc",
        },
        { name: "Hộ chiếu gốc (còn hạn tối thiểu 2 năm)", note: "Cần nộp gấp" },
      ],
      Hàn: [
        {
          name: "Hợp pháp hóa Lãnh sự Học bạ & Bằng tốt nghiệp",
          note: "Hồ sơ bắt buộc xin Visa thẳng",
        },
        {
          name: "Sổ tiết kiệm ngân hàng Hàn Quốc (K-Sure)",
          note: "Giá trị tối thiểu 10,000 USD",
        },
        {
          name: "Chứng minh công việc và thu nhập người bảo lãnh",
          note: "Theo mẫu quy định",
        },
      ],
    };
    ...
    if (docsContainer) {
      if (status === "Đã xuất cảnh" || status === "Đã trúng tuyển") {
        ...
        docsContainer.innerHTML = missingDocs
          .map(
            (doc) => `
    ...
    const mockTests = [
      {
        name: "Thi thử học kỳ năng lực ngoại ngữ",
        time: "08:30 - 11:30",
        date: "2026-08-25",
        location: "Phòng thi tầng 2",
      },
      {
        name: "Phỏng vấn thử ĐH/Trường Nhật ngữ",
        time: "14:00 - 17:00",
        date: "2026-09-02",
        location: "Phòng Lab VIP 1",
      },
    ];
  ...
  const showStudentPortalDebugError = (msg) => {
    ...
    if (!errDiv) {
      ...
      errDiv.style.cssText =
        "background: #fee2e2; border: 1px solid #ef4444; color: #b91c1c; padding: 1rem; margin: 1rem 4rem; border-radius: 8px; font-size: 0.85rem; font-family: monospace; white-space: pre-wrap; z-index: 99999; text-align: left;";
  ...
  const populateStudentProfileTab = (p) => {
    ...
    if (statusEl) {
      ...
      const sc =
        p.status === "Đang học"
          ? "active-badge"
          : p.status === "Chờ phỏng vấn"
            ? "pending-badge"
            : p.status === "Đã xuất cảnh"
              ? "completed-badge"
              : "inactive-badge";
      statusEl.className = `profile-status-badge ${sc}`;
    }
    ...
    if (p.flightDate) {
      const fd = p.flightDate.toDate
        ? p.flightDate.toDate()
        : new Date(p.flightDate);
      if (flightEl)
        flightEl.textContent = fd.toLocaleDateString("vi-VN", {
          day: "2-digit",
          month: "2-digit",
        ...
        try {
          ...
          if (doc.exists) {
            ...
            if (!studentQuery.empty) {
              ...
              console.log(
                `Self-healed: restored users document for student ${user.email}`,
              );
            } else {
  ...
  const openHrmProfile = (staff) => {
    ...
    document
      .querySelectorAll(".hrm-tab-content")
    ...
    document
      .querySelectorAll(".hrm-ptab")
      .forEach((t) => t.classList.remove("active"));
    document
      .querySelectorAll(".hrm-ptab-panel")
      .forEach((p) => p.classList.remove("active"));
    const firstTab = document.querySelector(
      '.hrm-ptab[data-ptab="ptab-general"]',
    );
  ...
  const closeHrmProfile = () => {
    ...
    document
      .querySelectorAll(".hrm-subtab")
      .forEach((t) => t.classList.remove("active"));
    const firstSubtab = document.querySelector(
      '.hrm-subtab[data-tab="hrm-staff-tab"]',
    );
  ...
  const populateHrmProfile = (s) => {
    ...
    const empCodeVal =
      globalIdx > 0 ? String(globalIdx).padStart(5, "0") : "--";
    ...
    if (emailEl) {
      ...
      emailEl.href = s.email ? `mailto:${s.email}` : "#";
    }
    const phoneEl = document.getElementById("profilePhone");
    if (phoneEl) {
      phoneEl.textContent = s.phone || "--";
      phoneEl.href = s.phone ? `tel:${s.phone}` : "#";
    }
    ...
    if (salEl) {
      salEl.textContent =
        salary > 0 ? salary.toLocaleString("vi-VN") + " đ" : "-- đ";
      salEl.style.color =
        salary > 0 ? "#059669" : "var(--color-text-muted, #6B6A67)";
    }
    ...
    if (wdEl) {
      wdEl.textContent = "...";
      wdEl.style.color = "var(--color-text-muted, #6B6A67)";
    }
    if (attEl) {
      attEl.textContent = "...";
      attEl.style.color = "var(--color-text-muted, #6B6A67)";
    }
    ...
    if (achEl) {
      ...
      if (achs.length) {
        achEl.innerHTML = achs
          .map((a) => `<span class="achievement-tag">⭐ ${esc(a)}</span>`)
          .join("");
        if (achBadge) {
          achBadge.textContent = achs.length;
          achBadge.style.display = "inline-flex";
        }
      } else {
        achEl.innerHTML =
          '<div class="achievement-empty">Chưa có thành tích được ghi nhận</div>';
    ...
    const _applyHrScore = (kpiDisplay, attPct) => {
      ...
      const gradeNote =
        totalScore >= 90
          ? "🏆 Xuất sắc — Nhân viên tiêu biểu"
          : totalScore >= 80
            ? "🌟 Tốt — Vượt kỳ vọng"
            : totalScore >= 70
              ? "👍 Khá — Đạt yêu cầu"
              : totalScore >= 60
                ? "⚠️ Trung bình — Cần cải thiện"
      ...
      if (scoreArcEl) {
        ...
        requestAnimationFrame(() => {
          scoreArcEl.style.transition =
            "stroke-dashoffset 1.1s cubic-bezier(.4,0,.2,1), stroke 0.3s";
          scoreArcEl.style.strokeDashoffset =
            326.7 - (totalScore / 100) * 326.7;
        });
      }
      ...
      if (hrKpiBar) {
        hrKpiBar.style.width = "0%";
        requestAnimationFrame(() => {
          hrKpiBar.style.transition = "width 1s ease";
          hrKpiBar.style.width = kpiDisplay + "%";
        });
      }
      if (hrAttBar) {
        hrAttBar.style.width = "0%";
        requestAnimationFrame(() => {
          hrAttBar.style.transition = "width 1s ease";
          hrAttBar.style.width = attPct + "%";
        });
      }
      if (hrKpiValEl) hrKpiValEl.textContent = kpiDisplay + "%";
      if (hrAttValEl) hrAttValEl.textContent = attPct + "%";
      if (hrNoteEl) {
        hrNoteEl.textContent = gradeNote;
        hrNoteEl.style.color = gradeColor;
      }
    };
    ...
    (async () => {
      ...
      const attPct =
        elapsedStd > 0
          ? Math.min(100, Math.round((actualDays / elapsedStd) * 100))
          : 0;
      ...
      if (wdEl) {
        wdEl.textContent =
          (actualDays % 1 === 0 ? actualDays : actualDays.toFixed(1)) +
          " / " +
          standardDays +
          " ngày";
        wdEl.style.color = "";
    ...
    if (skillTagsEl) {
      ...
      skillTagsEl.innerHTML = skills.length
        ? skills
            .map((sk) => `<span class="skill-tag">${esc(sk)}</span>`)
            .join("")
    ...
    if (leaveBarEl) {
      ...
      requestAnimationFrame(() => {
        leaveBarEl.style.width =
          leaveTotal > 0
            ? Math.min(100, Math.round((leaveUsed / leaveTotal) * 100)) + "%"
            : "0%";
      });
    }
      ...
      saveNotesBtn.onclick = async () => {
        try {
          await db
            .collection("hrm_staff")
            .doc(s.id)
            .update({ managerNotes: notesEl.value });
      ...
      adminPhotoInput.onchange = (e) => {
        ...
        reader.onload = async (ev) => {
          try {
            ...
            if (adminPhotoFrame)
              adminPhotoFrame.innerHTML = `<img src="${thumbUrl}" alt="${esc(s.name)}">`;
            s.photoUrl = thumbUrl;
            await db
              .collection("hrm_staff")
              .doc(s.id)
              .update({ photoUrl: thumbUrl });
            ...
            if (_hrmStorage) {
              try {
                ...
                const hdBlob = await new Promise((res) =>
                  hc.toBlob(res, "image/jpeg", 0.92),
                );
                if (hdBlob) {
                  const ref = _hrmStorage.ref(
                    `hrm_staff/${s.id}/photo/profile.jpg`,
                  );
                  ...
                  await db
                    .collection("hrm_staff")
                    .doc(s.id)
                    .update({ photoUrl: hdUrl });
                  s.photoUrl = hdUrl;
                  if (adminPhotoFrame)
                    adminPhotoFrame.innerHTML = `<img src="${hdUrl}" alt="${esc(s.name)}">`;
                  showToast("Ảnh đã nâng cấp Full HD!", "success");
                }
              } catch (_) {
                /* HD upgrade failed — thumbnail already saved */
              }
            }
          } catch (err) {
            showToast("Lỗi: " + err.message, "error");
    ...
    if (adminDocInput) {
      adminDocInput.onchange = () =>
        uploadHrmResumeDocs(s.id, adminDocInput, "admin");
    }
    ...
    if (adminCDocInput) {
      adminCDocInput.onchange = () =>
        uploadHrmContractDocs(s.id, adminCDocInput, "admin");
    }
    ...
    setText(
      "profileContractStartDate",
      fmtDate(s.contractStartDate || s.joinDate),
    );
    setText(
      "profileContractEndDate",
      s.contractEndDate ? fmtDate(s.contractEndDate) : "Vô thời hạn",
    );
    setText(
      "profileContractStatus",
      s.status === "Đã nghỉ việc" ? "Hết hiệu lực" : "Đang hiệu lực",
    );
    ...
    setText(
      "profileAllowanceLunch",
      s.allowanceSalary ? fmtCurrency(s.allowanceSalary) : "0 đ",
    );
    ...
    if (btnSaveSal)
      btnSaveSal.onclick = async () => {
        ...
        try {
          ...
          setText(
            "profileAllowanceLunch",
            newAllowance ? fmtCurrency(newAllowance) : "0 đ",
          );
  ...
  const renderKpiTrendChart = async (staffId, currentKpi, currentAtt) => {
    ...
    for (const monthStr of months) {
      ...
      try {
        const doc = await db
          .collection("attendance")
          .doc(`${staffId}_${monthStr}`)
          .get();
        if (doc.exists && doc.data().days) {
          const { S } = calcStandardDays(monthStr);
          let actual = 0;
          Object.values(doc.data().days).forEach((v) => {
            if (v === "1") actual++;
            else if (v === "0.5") actual += 0.5;
          });
      ...
      const kpi =
        monthStr === months[5]
          ? currentKpi
          : att > 0
            ? Math.min(100, att + Math.round((Math.random() - 0.4) * 15))
            : 0;
    ...
    const pts = (arr) =>
      arr.map((v, i) => `${PAD + i * xStep},${toY(v)}`).join(" ");
    ...
    monthsEl.innerHTML = labels
      .map((l) => `<span class="kpi-trend-month">${l}</span>`)
      .join("");
  };
  ...
  const loadHrmActivityLog = async (staffId) => {
    ...
    try {
      ...
      if (snap.empty) {
        logEl.innerHTML =
          '<div class="activity-item-empty">Chưa có hoạt động nào</div>';
        return;
      }
      logEl.innerHTML = snap.docs
        .map((doc) => {
          const d = doc.data();
      ...
      logEl.innerHTML =
        '<div class="activity-item-empty">Không thể tải nhật ký</div>';
    }
  };
  ...
  const loadHrmResumeDocs = async (staffId, prefix) => {
    const tbody = document.getElementById(
      prefix === "admin" ? "adminResumeDocsTbody" : "spResumeDocsTbody",
    );
    if (!tbody) return;
    ...
    try {
      ...
      if (snap.empty) {
        const emptyId =
          prefix === "admin" ? "adminResumeDocsEmpty" : "spResumeDocsEmpty";
  ...
  const uploadHrmResumeDocs = async (staffId, input, prefix) => {
    ...
    const progressEl = document.getElementById(
      prefix === "admin" ? "adminResumeDocProgress" : "spResumeDocProgress",
    );
    const msgEl = document.getElementById(
      prefix === "admin" ? "adminResumeDocMsg" : "spResumeDocMsg",
    );
  ...
  const _contractDocIds = (prefix) => ({
    tbody:
      prefix === "admin" ? "adminContractDocsTbody" : "spContractDocsTbody",
    progress:
      prefix === "admin" ? "adminContractDocProgress" : "spContractDocProgress",
    msg: prefix === "admin" ? "adminContractDocMsg" : "spContractDocMsg",
    empty:
      prefix === "admin" ? "adminContractDocsEmpty" : "spContractDocsEmpty",
  });
  ...
  const openContractDetailModal = (s) => {
    ...
    if (sub)
      sub.textContent =
        (s.name || "--") + (s.employeeCode ? ` · Mã ${s.employeeCode}` : "");
    const set2 = (id, val) => {
      const el = document.getElementById(id);
      if (el) el.textContent = val || "--";
    };
    ...
    set2(
      "cdEndDate",
      s.contractEndDate ? fmtD(s.contractEndDate) : "Vô thời hạn",
    );
  ...
  (() => {
    ...
    document
      .getElementById("btnCloseContractDetail")
      ?.addEventListener("click", closeCD);
    document
      .getElementById("btnCloseContractDetailFooter")
      ?.addEventListener("click", closeCD);
    document
  ...
  const setupHrmSubtabs = () => {
    document.querySelectorAll(".hrm-subtab").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (target === "hrm-staff-tab") {
          renderHrmKpi();
          subscribeToHrmStaff();
          renderHrmStaffList();
        } else if (target === "hrm-projects-tab") {
          renderHrmProjects();
        } else if (target === "hrm-payments-tab") {
          renderHrmPayments();
        } else if (target === "hrm-attendance-tab") {
          initHrmAttendanceTab();
        } else if (target === "hrm-payroll-tab") {
          initHrmPayrollTab();
        }
      });
    });
  ...
  const setupHrmModals = () => {
    ...
    document
      .getElementById("btnExportStaffExcel")
      ?.addEventListener("click", () => {
        exportStaffToExcel();
      });

    document
      .getElementById("btnOpenHrmStaffModal")
      ?.addEventListener("click", () => {
        document.getElementById("hrmStaffEditId").value = "";
        document.getElementById("hrmStaffForm").reset();
        document.getElementById("hrmStaffModalTitle").textContent =
          "+ THÊM NHÂN SỰ MỚI";
        ...
        if (emailElNew) {
          emailElNew.readOnly = false;
          emailElNew.style.opacity = "";
          emailElNew.style.cursor = "";
        }
        if (staffModal) staffModal.style.display = "flex";
      });
    document
      .getElementById("btnCloseHrmStaffModal")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("btnOpenHrmProjectModal")
      ?.addEventListener("click", () => {
        document.getElementById("hrmProjectEditId").value = "";
        document.getElementById("hrmProjectForm").reset();
        document.getElementById("hrmProjectModalTitle").textContent =
          "+ THÊM DỰ ÁN MỚI";
        if (projectModal) projectModal.style.display = "flex";
      });
    document
      .getElementById("btnCloseHrmProjectModal")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("btnBackToHrmList")
      ?.addEventListener("click", closeHrmProfile);

    document
      .getElementById("btnUpdateProfileEdit")
      ?.addEventListener("click", () => {
        if (_currentProfileStaff) editHrmStaff(_currentProfileStaff);
      });

    document
      .getElementById("btnProfileWorkEdit")
      ?.addEventListener("click", () => {
        if (_currentProfileStaff) editHrmStaff(_currentProfileStaff);
      });
    ...
    document
      .getElementById("btnHrScoreUpdate")
      ?.addEventListener("click", () => {
    ...
    const recalcKpiPreview = () => {
      ...
      const m =
        KPI_MILESTONES.find((x) => x.value === _kpiAttVal) || KPI_MILESTONES[0];
      const t =
        KPI_MILESTONES.find((x) => x.value === _kpiTaskVal) ||
        KPI_MILESTONES[0];

      if (el("kpiAttScore")) {
        el("kpiAttScore").textContent = _kpiAttVal + "%";
        el("kpiAttScore").style.color = m.color;
      }
      if (el("kpiAttBar")) el("kpiAttBar").style.width = _kpiAttVal + "%";
      if (el("kpiTaskScore")) {
        el("kpiTaskScore").textContent = _kpiTaskVal + "%";
        el("kpiTaskScore").style.color = t.color;
      }
      ...
      if (el("kpiGradePreview"))
        el("kpiGradePreview").textContent = `(${grade})`;
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        ...
        const monthStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}`;
        const el = (id) => document.getElementById(id);
        if (el("kpiEvalMonthLabel"))
          el("kpiEvalMonthLabel").textContent =
            `Nhân viên: ${s.name} · Tháng ${now.getMonth() + 1}/${now.getFullYear()}`;
    ...
    document
      .getElementById("btnCloseKpiEval")
      ?.addEventListener("click", () => {
        document.getElementById("kpiEvalModal").style.display = "none";
      });
    document
      .getElementById("btnCancelKpiEval")
      ?.addEventListener("click", () => {
        document.getElementById("kpiEvalModal").style.display = "none";
      });

    document
      .getElementById("btnApproveKpi")
      ?.addEventListener("click", async () => {
        if (!_currentProfileStaff) return;
        if (_kpiAttVal === 0 && _kpiTaskVal === 0) {
          showToast("Vui lòng chọn mức đánh giá cho từng tiêu chí", "error");
          return;
        }
        const s = _currentProfileStaff;
        const now = new Date();
        ...
        try {
          ...
          logHrmActivity(
            s.id,
            `Duyệt KPI tháng ${monthStr}: ${finalKpi}/100 (CN ${_kpiAttVal}%, CV ${_kpiTaskVal}%)`,
          );
          showToast(
            `KPI tháng ${monthStr} đã duyệt: ${finalKpi}/100`,
            "success",
          );
        } catch (e) {
    ...
    document
      .getElementById("btnGoAttendance")
      ?.addEventListener("click", () => {
        closeHrmProfile();
        const attTab = document.querySelector(
          '.hrm-subtab[data-tab="hrm-attendance-tab"]',
        );
        if (attTab) attTab.click();
      });

    document
      .getElementById("btnViewActivityLog")
      ?.addEventListener("click", () => {
  ...
  const setupHrmForms = () => {
    document
      .getElementById("hrmStaffSearch")
      ?.addEventListener("input", () => renderHrmStaffList());
    document
      .getElementById("hrmStaffDeptFilter")
      ?.addEventListener("change", () => renderHrmStaffList());

    // Attendance month controls
    document
      .getElementById("hrmAttendanceMonth")
      ?.addEventListener("change", () => {
        subscribeToHrmAttendance(
          document.getElementById("hrmAttendanceMonth").value,
        );
      });
    document
      .getElementById("btnAttPrevMonth")
      ?.addEventListener("click", () => {
        shiftMonthInput("hrmAttendanceMonth", -1, subscribeToHrmAttendance);
      });
    document
      .getElementById("btnAttNextMonth")
      ?.addEventListener("click", () => {
        shiftMonthInput("hrmAttendanceMonth", 1, subscribeToHrmAttendance);
      });
    ...
    document
      ...
      ?.addEventListener("submit", async (e) => {
        ...
        const data = {
          ...
          department: dept,
          jobTitle:
            document.getElementById("hrmStaffJobTitle")?.value || "Nhân viên",
          level: document.getElementById("hrmStaffLevel")?.value || "Cấp 1",
          workType:
            document.getElementById("hrmStaffWorkType")?.value || "Full-time",
          ...
          username:
            document.getElementById("hrmStaffUsername")?.value.trim() || "",
          hometown:
            document.getElementById("hrmStaffHometown")?.value.trim() || "",
          gender: document.getElementById("hrmStaffGender")?.value || "",
          maritalStatus:
            document.getElementById("hrmStaffMarital")?.value || "",
          education:
            document.getElementById("hrmStaffEducation")?.value.trim() || "",
          emergencyContactName:
            document.getElementById("hrmStaffEmergencyName")?.value.trim() ||
            "",
          emergencyContactPhone:
            document.getElementById("hrmStaffEmergencyPhone")?.value.trim() ||
            "",
          emergencyContactRelation:
            document
              .getElementById("hrmStaffEmergencyRelation")
              ?.value.trim() || "",
          updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
        };
        ...
        try {
          if (editId) {
            ...
            try {
              ...
              if (authErr.message === "EMAIL_EXISTS") {
                ...
                try {
                  ...
                  showToast(
                    "Email này đã có tài khoản đăng nhập cũ — đang khôi phục hồ sơ nhân sự cho tài khoản đó...",
                    "info",
                  );
                } catch (signInErr) {
                  console.error(
                    "Lỗi đăng nhập khôi phục tài khoản cũ:",
                    signInErr,
                  );
                  showToast(
                    "Email này đã có tài khoản nhưng mật khẩu vừa nhập không khớp với mật khẩu cũ. Hãy nhập đúng mật khẩu cũ của tài khoản này, hoặc xóa tài khoản đó trong Firebase Console > Authentication rồi thử lại.",
                    "error",
                  );
                  return;
                }
            ...
            try {
              ...
              console.error(
                'Lỗi lưu Firestore (kiểm tra Security Rules cho "users"/"hrm_staff"):',
                firestoreErr,
              );
              showToast(
                "Tài khoản đăng nhập đã tạo nhưng KHÔNG lưu được vào Firestore (lỗi quyền: " +
    ...
    document
      ...
      ?.addEventListener("submit", async (e) => {
        ...
        const data = {
          ...
          progress:
            parseInt(document.getElementById("hrmProjectProgress").value) || 0,
          leader: document.getElementById("hrmProjectLeader").value.trim(),
          tasksCount:
            parseInt(document.getElementById("hrmProjectTasks").value) || 0,
  ...
  const renderHrmKpi = async () => {
    ...
    try {
      ...
      const activeCount = allStaff.filter(
        (s) => s.status === "Đang làm việc",
      ).length;
      const leaveCount = allStaff.filter(
        (s) => s.status === "Nghỉ phép",
      ).length;
      const resignCount = allStaff.filter(
        (s) => s.status === "Đã nghỉ việc",
      ).length;
      ...
      grid.innerHTML = depts
        .map((dept) => {
          ...
          const deptActive = deptAll.filter(
            (s) => s.status === "Đang làm việc",
          ).length;
          const deptLeave = deptAll.filter(
            (s) => s.status === "Nghỉ phép",
          ).length;
          const pct =
            deptTotal > 0 ? Math.round((deptActive / deptTotal) * 100) : 0;
          return `
      ...
      .onSnapshot(
        (snap) => {
          ...
          if (_currentProfileStaff) {
            const updated = hrmStaffCache.find(
              (x) => x.id === _currentProfileStaff.id,
            );
            if (updated) {
  ...
  const exportStaffToExcel = () => {
    if (!window.XLSX) {
      showToast(
        "Thư viện xuất Excel chưa tải xong, vui lòng thử lại!",
        "error",
      );
      return;
    }
  ...
  const renderHrmStaffList = () => {
    ...
    tableBody.innerHTML = "";
    try {
      const query = (document.getElementById("hrmStaffSearch")?.value || "")
        .trim()
        .toLowerCase();
      const deptFilter =
        document.getElementById("hrmStaffDeptFilter")?.value || "All";
      ...
      const globalIndexMap = new Map(
        hrmStaffCache.map((s, i) => [s.id, i + 1]),
      );
      ...
      staffList.forEach((s) => {
        ...
        tr.querySelector(".btn-view-hrm-staff")?.addEventListener("click", () =>
          openHrmProfile(s),
        );
        tr.querySelector(".btn-edit-hrm-staff")?.addEventListener("click", () =>
          editHrmStaff(s),
        );
        tr.querySelector(".btn-del-hrm-staff")?.addEventListener("click", () =>
          deleteHrmStaff(s.id, s.name),
        );
        tableBody.appendChild(tr);
      });
    } catch (err) {
  ...
  const renderHrmAttendanceDayView = (monthStr, day) => {
    ...
    head.innerHTML =
      '<tr><th class="att-name-cell">NHÂN VIÊN</th><th>PHÒNG BAN</th><th>TRẠNG THÁI</th><th>GIỜ CHẤM CÔNG</th><th>IP</th></tr>';

    if (hrmStaffCache.length === 0) {
      body.innerHTML =
        '<tr><td colspan="5" style="text-align:center; padding:2rem; color:var(--text-muted);">Chưa có nhân sự nào.</td></tr>';
      return;
    }
  ...
  const bindAttendanceCellClicks = (monthStr) => {
    document
      .getElementById("hrmAttendanceBody")
      .querySelectorAll(".att-cell")
      .forEach((cell) => {
        cell.addEventListener("click", async () => {
          ...
          const nextIdx =
            (ATTENDANCE_STATUS_CYCLE.indexOf(current) + 1) %
            ATTENDANCE_STATUS_CYCLE.length;
  ...
  const renderHrmAttendanceTable = (monthStr) => {
    ...
    const dayFilter =
      document.getElementById("hrmAttendanceDayFilter")?.value || "";
    if (dayFilter) {
    ...
    if (hrmStaffCache.length === 0) {
      body.innerHTML =
        '<tr><td colspan="32" style="text-align:center; padding:2rem; color:var(--text-muted);">Chưa có nhân sự nào.</td></tr>';
      return;
    }
  ...
  const initHrmAttendanceTab = () => {
    ...
    if (dayFilter && !dayFilter.dataset.bound) {
      ...
      dayFilter.addEventListener("change", () =>
        renderHrmAttendanceTable(monthInput.value),
      );
    }
  };
  ...
  const renderPayrollTable = async (monthStr) => {
    ...
    if (!hrmStaffCache.length) {
      body.innerHTML =
        '<tr><td colspan="8" style="text-align:center;padding:2rem;color:var(--text-muted);">Chưa có nhân sự nào.</td></tr>';
      return;
    }

    // Lấy giờ chuẩn từ input
    const timeInStd =
      document.getElementById("payrollTimeIn")?.value || "07:45";
    const timeOutStd =
      document.getElementById("payrollTimeOut")?.value || "17:00";
    ...
    try {
      ...
      console.error("Lỗi tải checkin_logs:", e);
    }
    ...
    hrmStaffCache.forEach((s) => {
      ...
      logs.forEach((l) => {
        ...
        workedDays++;
        const cin = l.checkin_time.toDate
          ? l.checkin_time.toDate()
          : new Date(l.checkin_time);
        if (
          cin.getHours() > inH ||
          (cin.getHours() === inH && cin.getMinutes() > inM)
        )
          lateDays++;
        if (l.checkout_time) {
          const cout = l.checkout_time.toDate
            ? l.checkout_time.toDate()
            : new Date(l.checkout_time);
          if (
            cout.getHours() < outH ||
            (cout.getHours() === outH && cout.getMinutes() < outM)
          )
            earlyDays++;
        }
      });
      ...
      const renderSalaryCell = (otWd, otWe) => {
        if (!lcb)
          return '<span style="color:var(--text-muted);font-weight:400;font-size:0.8rem;">Chưa có LCB</span>';
        const { dayPay, otWdPay, otWePay, penalty, total } = calcSalary(
          otWd,
          otWe,
        );
        ...
        return `
          ...
          ${
            hasBonuses
              ? `<div style="font-size:0.68rem;color:var(--text-muted);margin-top:2px;line-height:1.5;">
            NC: ${fmtMoney(dayPay)}
      ...
      const penaltyColor =
        lateDays + earlyDays > 0 ? "#EF4444" : "var(--text-muted)";
  ...
  const editHrmStaff = (s) => {
    ...
    document.getElementById("hrmStaffStatus").value =
      s.status || "Đang làm việc";
  ...
  const renderHrmProjects = async () => {
    ...
    try {
      const snap = await db
        .collection("hrm_projects")
        .orderBy("createdAt", "desc")
        .get();
      if (snap.empty) {
        grid.innerHTML =
          '<div class="hrm-empty-state"><svg viewBox="0 0 24 24"><path d="M19,3H5C3.89,3 3,3.89 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V5C21,3.89 20.1,3 19,3M19,19H5V5H19V19M17,12H7V10H17V12M17,16H7V14H17V16M14,8H7V6H14V8Z"/></svg><h4>Chưa có dự án nào</h4><p>Hãy tạo dự án mới để bắt đầu quản lý.</p></div>';
        return;
      }
      snap.forEach((doc) => {
        const p = doc.data();
        ...
        card
          .querySelector(".btn-edit-hrm-proj")
          ?.addEventListener("click", () => {
            document.getElementById("hrmProjectEditId").value = p.id;
            document.getElementById("hrmProjectName").value = p.name;
            document.getElementById("hrmProjectDesc").value =
              p.description || "";
            document.getElementById("hrmProjectStatus").value = p.status;
            document.getElementById("hrmProjectProgress").value =
              p.progress || 0;
            document.getElementById("hrmProjectLeader").value = p.leader || "";
            document.getElementById("hrmProjectTasks").value =
              p.tasksCount || 0;
            if (document.getElementById("hrmProjectScale"))
              document.getElementById("hrmProjectScale").value =
                p.scale || "Nhỏ";
            if (document.getElementById("hrmProjectDeadline"))
              document.getElementById("hrmProjectDeadline").value =
                p.deadline || "";
            document.getElementById("hrmProjectModalTitle").textContent =
              "✏️ SỬA DỰ ÁN";
        ...
        card
          .querySelector(".btn-del-hrm-proj")
          ?.addEventListener("click", async () => {
        ...
        card
          .querySelector(".btn-detail-hrm-proj")
          ?.addEventListener("click", () => openProjectDetail(p));
        grid.appendChild(card);
      });
    } catch (err) {
  ...
  const loadPdMembers = async (projectId) => {
    ...
    if (!list) return;
    list.innerHTML =
      '<div style="font-size:0.78rem;color:var(--text-muted);text-align:center;padding:1.5rem 0;">Đang tải...</div>';
    try {
      const snap = await db
        .collection("hrm_projects")
        .doc(projectId)
      ...
      if (snap.empty) {
        list.innerHTML =
          '<div style="font-size:0.8rem;color:var(--text-muted);text-align:center;padding:2rem 0;font-style:italic;">Chưa có thành viên. Nhấn "+ Thêm thành viên" để bắt đầu.</div>';
        return;
      }
      list.innerHTML = "";
      snap.forEach((doc) => {
        ...
        card.querySelectorAll(".pd-star").forEach((star) => {
          ...
          star.addEventListener("mouseenter", () => {
            ...
            card.querySelectorAll(".pd-star").forEach((s, i) => {
              s.style.color = i < hv ? "#F59E0B" : "#D1D5DB";
            });
          });
        ...
        // Save eval
        card
          .querySelector(".pd-save-eval")
          ?.addEventListener("click", async () => {
            const evaluation =
              card.querySelector(".pd-eval-text")?.value.trim() || "";
            try {
              await db
                .collection("hrm_projects")
                .doc(projectId)
                .collection("members")
                .doc(doc.id)
                .update({ rating: currentRating, evaluation });
      ...
      list.innerHTML =
        '<div style="color:#EF4444;font-size:0.8rem;padding:1rem;">Lỗi tải thành viên.</div>';
    }
  };
  ...
  document.getElementById("btnAddPdMember")?.addEventListener("click", () => {
    ...
    if (pickerEl) {
      ...
      if (!staffList.length) {
        pickerEl.innerHTML =
          '<div style="font-size:0.78rem;color:var(--text-muted);font-style:italic;padding:0.5rem;">Chưa có nhân viên nào trong hệ thống.</div>';
      } else {
        pickerEl.innerHTML = staffList
          .map((s) => {
            const ini = (s.name || "?")
              .split(" ")
              .map((n) => n[0])
  ...
  document
    .getElementById("btnCancelPdMember")
    ?.addEventListener("click", () => {
      document.getElementById("pdAddMemberForm").style.display = "none";
    });
  document
    .getElementById("btnSavePdMember")
    ?.addEventListener("click", async () => {
      ...
      if (!name) {
        showToast("Vui lòng chọn một thành viên từ danh sách", "error");
        return;
      }
  ...
  const renderHrmPayments = async () => {
    ...
    try {
      const snap = await db
        .collection("hrm_payments")
        .orderBy("createdAt", "desc")
        .get();
      if (snap.empty) {
        tbody.innerHTML =
          '<tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--text-muted);font-size:0.85rem;">Chưa có đề xuất thanh toán nào.</td></tr>';
        return;
      }
      ...
      snap.forEach((doc) => {
        ...
        tr.querySelector(".btn-approve-pay")?.addEventListener(
          "click",
          async () => {
            try {
              await db
        ...
        tr.querySelector(".btn-reject-pay")?.addEventListener(
          "click",
          async () => {
            try {
              await db
        ...
        tr.querySelector(".btn-del-pay")?.addEventListener(
          "click",
          async () => {
  ...
  const openCrmProfile = (customer) => {
    ...
    document
      .querySelectorAll(".crm-ptab")
      .forEach((t) => t.classList.remove("active"));
    document
      .querySelectorAll(".crm-ptab-panel")
      .forEach((p) => p.classList.remove("active"));
  ...
  const closeCrmProfile = () => {
    ...
    const targetTab = activeSubtab
      ? activeSubtab.getAttribute("data-tab")
      : "crm-overview-tab";
  ...
  const populateCrmProfile = (c) => {
    ...
    if (av) {
      ...
      av.style.background = avatarColors[colorIdx];
    }

    if (getEl("crmProfileName"))
      getEl("crmProfileName").textContent = c.name || "--";
    if (getEl("crmProfileCode"))
      getEl("crmProfileCode").textContent = c.code || "--";
    if (getEl("crmProfileCountry"))
      getEl("crmProfileCountry").textContent = c.country || "--";
    if (getEl("crmProfileEmail"))
      getEl("crmProfileEmail").textContent = c.email || "--";
    if (getEl("crmProfilePhone"))
      getEl("crmProfilePhone").textContent = c.phone || "--";
    if (getEl("crmProfileCountryVal"))
      getEl("crmProfileCountryVal").textContent = c.country || "--";
    ...
    if (getEl("crmProfileCreatedAt"))
      getEl("crmProfileCreatedAt").textContent = createdStr;
  ...
  const setupDocUpload = () => {
    ...
    input.addEventListener("change", async () => {
      ...
      for (const file of files) {
        if (msg)
          msg.textContent = `Đang lưu: ${file.name} (${done + 1}/${files.length})`;
        try {
          let url = "";
          let storagePath = "";
  ...
  const renderCrmOverview = () => {
    ...
    const convRate =
      total > 0 ? Math.round(((selected + active) / total) * 100) : 0;
    ...
    if (recentEl) {
      ...
      const badgeCls = {
        "Đang học": "crm-badge-active",
        "Chờ phỏng vấn": "crm-badge-waiting",
        "Đang làm hồ sơ": "crm-badge-processing",
        "Đã trúng tuyển": "crm-badge-selected",
      };
      recentEl.innerHTML =
        sorted.length > 0
          ? sorted
          ...
          <div class="crm-recent-item">
            ...
            <span class="crm-recent-badge crm-pill ${bc}">${c.status || ""}</span>
              ...
              .join("")
          : '<p style="color:var(--text-muted);font-size:0.82rem;text-align:center;padding:1rem">Chưa có dữ liệu.</p>';
    }
  };
  ...
  const renderCrmCustomers = (resetPage = false) => {
    ...
    const search = (document.getElementById("crmSearchInput")?.value || "")
      ...
      .trim();
    const countryF =
      document.getElementById("crmCountryFilter")?.value || "All";
    ...
    const filtered = _allCrmCustomers.filter((c) => {
      ...
      if (
        search &&
        !`${c.name} ${c.email} ${c.code}`.toLowerCase().includes(search)
      )
        return false;
      return true;
    });
    ...
    const pageData = filtered.slice(
      (crmCustomerPage - 1) * PAGE_SIZE,
      crmCustomerPage * PAGE_SIZE,
    );
    const globalOffset = (crmCustomerPage - 1) * PAGE_SIZE;

    const globalCrmIndexMap = new Map(
      _allCrmCustomers.map((c, i) => [c.id, 30001 + i]),
    );
    const avColors = [
      "#2563EB",
      "#7C3AED",
      ...
      sel.addEventListener("change", async () => {
        ...
        try {
          await db
            .collection("students")
            .doc(sel.dataset.id)
            .update({ advisor: sel.value });
        } catch (e) {
          console.error("Lỗi lưu advisor:", e);
        }
      });
    });
    ...
    tbody.querySelectorAll(".btn-view-crm").forEach((btn) => {
      btn.addEventListener("click", () =>
        openCrmProfile(pageData[parseInt(btn.dataset.fidx)]),
      );
    });
  ...
  const CRM_OLD_STATUS_COLORS = {
    ...
    "N3": { bg: "rgba(219,39,119,0.1)", color: "#DB2777" },
    "N4": { bg: "rgba(147,51,234,0.1)", color: "#9333EA" },
    "N5": { bg: "rgba(8,145,178,0.1)", color: "#0891B2" },
  ...
  const renderCrmOldCustomers = (resetPage = false) => {
    ...
    const search = (document.getElementById("crmOldSearchInput")?.value || "")
      .toLowerCase()
      .trim();
    const countryF =
      document.getElementById("crmOldCountryFilter")?.value || "All";
    const statusF =
      document.getElementById("crmOldStatusFilter")?.value || "All";
    ...
    const pageData = filtered.slice(
      (crmOldCustomerPage - 1) * PAGE_SIZE,
      crmOldCustomerPage * PAGE_SIZE,
    );
  ...
  const saveSourceRevenue = async (docId, value) => {
    ...
    try {
      await db
        .collection("students")
        .doc(docId)
        .update({ revenue: Number(value) || 0 });
    } catch (e) {
      console.error("Lỗi lưu doanh thu:", e);
    }
  };
  ...
  const renderRevenueBarChart = (forceYear) => {
    ...
    const SEED = [
      42e6, 68e6, 55e6, 91e6, 78e6, 105e6, 88e6, 120e6, 97e6, 135e6, 112e6, 0,
    ];
    ...
    const fmtFull = (v) =>
      v > 0 ? Number(v).toLocaleString("vi-VN") + " đ" : "0 đ";
  ...
  const showBarDayPopup = (monthIdx, year, anchorEl) => {
    ...
    const rows = sortedDays.length
      ? sortedDays
          .map((day) => {
            const info = days[day];
            const names =
              info.students.slice(0, 3).join(", ") +
              (info.students.length > 3
                ? `… +${info.students.length - 3}`
                : "");
    ...
    const top = rect.top > ph + 12 ? rect.top - ph - 8 : rect.bottom + 8;

    popup.style.left = left + "px";
    popup.style.top = Math.max(8, top) + "px";

    document
      .getElementById("crmBarPopupClose")
      ?.addEventListener("click", () => {
  ...
  const renderCrmSource = (resetPage = false) => {
    ...
    const search = (document.getElementById("srcSearchInput")?.value || "")
      .toLowerCase()
      .trim();
    const countryF =
      document.getElementById("srcCountryFilter")?.value || "All";
    ...
    const pageData = filtered.slice(
      (crmSourcePage - 1) * PAGE_SIZE,
      crmSourcePage * PAGE_SIZE,
    );
    ...
    tbody.querySelectorAll(".src-view-detail-btn").forEach((btn) => {
      btn.addEventListener("click", () =>
        openSourceDetail(pageData[parseInt(btn.dataset.idx)]),
      );
    });
      ...
      btn.addEventListener("click", async () => {
        ...
        ["1", "2", "3", "Cuoi"].forEach((suffix) => {
          const keyField =
            suffix === "Cuoi"
              ? "ngay_thi_cuoi"
              : `ngay_thi_${suffix.toLowerCase()}`;
          const resField =
            suffix === "Cuoi"
              ? "ket_qua_cuoi"
              : `ket_qua_${suffix.toLowerCase()}`;
    ...
    tbody.querySelectorAll(".src-delete-btn").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        try {
          ...
          _allCrmCustomers = _allCrmCustomers.filter(
  ...
  const loadRevEntries = async (studentId) => {
    ...
    try {
      ...
      if (snap.empty) {
        ...
        document.getElementById("srcDetailEntryCount").textContent =
          "0 đợt thanh toán";
        return;
      }
      let total = 0;
      ...
      document.getElementById("srcDetailTotalRevenue").textContent =
        fmtVND(total);
      document.getElementById("srcDetailEntryCount").textContent =
        `${rows.length} đợt thanh toán`;
      ...
      list.querySelectorAll(".rev-delete-btn").forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          await db
            .collection("students")
            .doc(_currentSrcStudent.id)
            .collection("revenueEntries")
            .doc(btn.dataset.entryid)
            .delete();
  ...
  const openSourceDetail = (student) => {
    ...
    const avSeed = (student.name || "")
      .split("")
      .reduce((a, c) => a + c.charCodeAt(0), 0);
    const ini = (student.name || "HV")
      .split(" ")
      .map((w) => w[0])
      .filter(Boolean)
      .slice(-2)
    ...
    if (av) {
      av.textContent = ini;
      av.style.background = avColors[avSeed % avColors.length];
    }
  ...
  const setupSourceDetailModal = () => {
    ...
    document
      .getElementById("btnCloseSourceDetail")
      ?.addEventListener("click", closeModal);
    document
      .getElementById("btnCloseSourceDetailFooter")
      ?.addEventListener("click", closeModal);
    ...
    document.getElementById("btnAddRevEntry")?.addEventListener("click", () => {
      ...
      if (form.style.display === "block") {
        ...
        if (d)
          d.value = `${today.getFullYear()}-${padZ(today.getMonth() + 1)}-${padZ(today.getDate())}`;
    ...
    document
      .getElementById("btnCancelRevEntry")
      ?.addEventListener("click", () => {
        document.getElementById("revEntryForm").style.display = "none";
      });

    document
      .getElementById("btnSaveRevEntry")
      ?.addEventListener("click", async () => {
        const amount = parseFloat(
          document.getElementById("revEntryAmount")?.value || 0,
        );
        const date = document.getElementById("revEntryDate")?.value;
        const note = document.getElementById("revEntryNote")?.value.trim();
        if (!amount || !date) {
          showToast("Vui lòng nhập số tiền và ngày", "error");
          return;
        }
    ...
    document
      .getElementById("btnEditSourceStudent")
      ?.addEventListener("click", async () => {
    ...
    try {
      ...
      list.querySelectorAll(".note-delete-btn").forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          await db
            .collection("students")
            .doc(_currentCrmCustomer.id)
            .collection("notes")
            .doc(btn.dataset.noteid)
            .delete();
  ...
  const populateCrmStaffProfile = (s) => {
    ...
    if (positions) {
      const positionText = s.jobTitle || s.position || "";
      positions.textContent = positionText
        ? `${positionText} • ${s.department || ""}`
        : "--";
    }
    ...
    if (incomeEl)
      incomeEl.textContent =
        s.salary > 0 ? Number(s.salary).toLocaleString("vi-VN") + " đ" : "-- đ";

    const seed = (s.id || s.name || "x")
      .split("")
      .reduce((a, c) => a + c.charCodeAt(0), 0);
    const companyDebt =
      Math.round(((seed * 1234567) % 25000000) / 1000000) * 1000000;
    const personalDebt =
      Math.round(((seed * 89012) % 4000000) / 500000) * 500000;
    setText(
      "crmPTotalDebt",
      (companyDebt + personalDebt).toLocaleString("vi-VN") + " đ",
    );
    ...
    setText(
      "crmProfileContractStartDate",
      fmtDate(s.contractStartDate || s.joinDate),
    );
    setText(
      "crmProfileContractEndDate",
      s.contractEndDate ? fmtDate(s.contractEndDate) : "Vô thời hạn",
    );
    setText(
      "crmProfileContractStatus",
      s.status === "Đã nghỉ việc" ? "Hết hiệu lực" : "Đang hiệu lực",
    );
  ...
  const openCrmStaffProfile = (s) => {
    ...
    document
      .querySelectorAll(".crmsp-tab")
      .forEach((t) => t.classList.remove("active"));
    document
      .querySelectorAll(".crmsp-panel")
      .forEach((p) => p.classList.remove("active"));
    const firstTab = document.querySelector(
      '.crmsp-tab[data-ctab="cptab-general"]',
    );
    if (firstTab) firstTab.classList.add("active");
  ...
  const renderCrmStaff = (resetPage = false) => {
    ...
    if (!tbody) return;
    const search = (
      document.getElementById("crmStaffSearch")?.value || ""
    ).toLowerCase();
    const dept = document.getElementById("crmStaffDeptFilter")?.value || "All";
    const status =
      document.getElementById("crmStaffStatusFilter")?.value || "All";
    ...
    if (counter)
      counter.textContent = `${filtered.length} / ${_allCrmStaff.length} nhân viên`;

    if (!filtered.length) {
      tbody.innerHTML =
        '<tr><td colspan="7" style="text-align:center;color:var(--text-muted);padding:2rem">Không tìm thấy nhân viên nào</td></tr>';
    ...
    const pageData = filtered.slice(
      (crmStaffPage - 1) * PAGE_SIZE,
      crmStaffPage * PAGE_SIZE,
    );
    ...
    tbody.querySelectorAll(".crm-row-action-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        if (btn.classList.contains("view")) openCrmStaffProfile(staff);
          ...
          deleteHrmStaff(staff.id, staff.name);
      });
    });
  ...
  const iocLoadPrefs = () => {
    try {
      _iocFavThreads = new Set(
        JSON.parse(localStorage.getItem(iocFavKey()) || "[]"),
      );
      _iocMutedThreads = new Set(
        JSON.parse(localStorage.getItem(iocMuteKey()) || "[]"),
      );
    } catch (_) {}
  };
  const iocSavePrefs = () => {
  ...
  const iocIsMine = (msg) => {
    ...
    return (
      (msg.senderEmail &&
        me.email &&
        msg.senderEmail.toLowerCase() === me.email.toLowerCase()) ||
      (msg.senderName &&
        me.name &&
        msg.senderName.toLowerCase() === me.name.toLowerCase())
    );
  };
  ...
  const iocOpenThread = (id, name, av, color, type = "dm", desc = "") => {
    ...
    localStorage.setItem(
      `ioc_lastread_${id}_${currentUser?.email}`,
      Date.now(),
    );
    ...
    if (avEl) {
      avEl.textContent = _iocActiveThread.av;
      avEl.style.background = _iocActiveThread.color;
    }
    ...
    if (infoAvEl) {
      infoAvEl.textContent = _iocActiveThread.av;
      infoAvEl.style.background = _iocActiveThread.color;
    }
    if (infoNameEl) {
  ...
  const iocBuildConvItem = (
    ...
    const unreadHtml =
      unread > 0
        ? `<span class="ioc-conv-unread">${unread > 9 ? "9+" : unread}</span>`
        : "";
  ...
  const iocRenderConvList = () => {
    ...
    const search = (
      document.getElementById("crmChatSearch")?.value || ""
    ).toLowerCase();
    ...
    const { text: gText, time: gTime } = groupIsActive
      ...
      : {
          text: _iocDmPreviews["group-global"]?.text || "Chưa có tin nhắn",
          time: _iocDmPreviews["group-global"]?.time || "",
        };
    const groupUnread = groupIsActive
      ? 0
      : _iocDmPreviews["group-global"]?.unread || 0;
    const groupItem = iocBuildConvItem(
      "group-global",
      "N",
      "#6366F1",
      "Nhóm Nội bộ Aladdin",
    ...
    const dmItemsHtml = filtered.slice(0, 20).map((u) => {
      ...
      const dmId =
        "dm-" +
        [
          currentUser?.uid || currentUser?.email || "me",
          u.id || u.uid || u.email,
        ]
          .sort()
          .join("__");
      const active = _iocActiveThread.id === dmId;
      const { text, time } = active
        ? getLastMsgPreview(_iocMsgs)
        : {
            text:
              _iocDmPreviews[dmId]?.text ||
              u.position ||
              u.department ||
              "Nhân viên",
            time: _iocDmPreviews[dmId]?.time || "",
          };
  ...
  const iocRenderMessages = (msgs) => {
    ...
    if (!msgs.length) {
      container.innerHTML =
        '<div class="crm-chat-loading">Chưa có tin nhắn. Hãy bắt đầu cuộc trò chuyện!</div>';
      iocRenderConvList();
      return;
    }
    ...
    msgs.forEach((msg) => {
      ...
      if (dateStr !== lastDate) {
        parts.push(
          `<div class="ioc-date-divider"><span>${dateStr}</span></div>`,
        );
        lastDate = dateStr;
        lastSender = "";
      }
      ...
      if (msg.recalled) {
        ...
        const editBadge = msg.edited
          ? '<span class="ioc-msg-edited">đã sửa</span>'
          : "";
        const pinBadge = msg.pinned
          ? '<span class="ioc-msg-pinned-badge">📌 Đã ghim</span>'
          : "";
        let media = "";
        if (msg.imageUrl) {
        ...
        const text = msg.content
          ? `<span class="ioc-msg-text">${esc(msg.content)}</span>`
          : "";
    ...
    container.querySelectorAll(".ioc-msg-action-btn").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        if (btn.dataset.act === "reply") {
          iocSetReply(btn.dataset.msgid);
          return;
        }
        if (btn.dataset.act === "more")
          iocShowCtxMenu(e, btn.dataset.msgid, row.dataset.mine === "true");
      });
    });
    container.querySelectorAll(".ioc-msg-bubble[data-msgid]").forEach((b) => {
      b.addEventListener("contextmenu", (e) => {
        ...
        iocShowCtxMenu(
          e,
          b.dataset.msgid,
          b.closest(".ioc-msg-row").dataset.mine === "true",
        );
      });
    });
  ...
  const iocShowCtxMenu = (e, msgId, isMine) => {
    ...
    if (pinBtn && msg) {
      const tn = [...pinBtn.childNodes].find(
  ...
  const iocSetReply = (msgId) => {
    ...
    document.getElementById("iocReplySender").textContent =
      _iocReplyTo.senderName;
  ...
  const iocUpdatePinnedCount = () => {
    ...
    if (el)
      el.textContent = _iocMsgs.filter((m) => m.pinned && !m.recalled).length;
  };
    ...
    list.innerHTML = pinned.length
      ? pinned
        ...
        <div class="ioc-pinned-item-text">${esc(m.content || "")}</div>
          ...
          .join("")
      : '<div style="padding:1rem;color:var(--text-muted);font-size:0.82rem;text-align:center">Chưa có tin nhắn được ghim</div>';
  };
  ...
  const iocShowMedia = (type) => {
    ...
    if (!grid) return;
    const filtered = _iocMsgs.filter(
      (m) => !m.recalled && (type === "image" ? !!m.imageUrl : !!m.fileName),
    );
    if (!filtered.length) {
    ...
    if (type === "image") {
      grid.innerHTML = filtered
        .map(
          (m) =>
            `<img src="${m.imageUrl}" class="ioc-media-thumb" alt="" title="${esc(m.senderName || "")}">`,
        )
        .join("");
  ...
  const iocToggleFavorite = () => {
    ...
    iocRenderConvList();
    showToast(
      _iocFavThreads.has(id) ? "Đã thêm vào yêu thích!" : "Đã bỏ yêu thích!",
      "success",
    );
  };
  ...
  const iocToggleNotify = () => {
    ...
    iocUpdateNotifyBtn();
    showToast(
      _iocMutedThreads.has(id) ? "Đã tắt thông báo!" : "Đã bật thông báo!",
      "success",
    );
  };
  ...
  const iocRenameGroup = () => {
    ...
    const save = async () => {
      ...
      editBtn.className = "ioc-info-edit-btn";
      editBtn.id = "btnIocRenameGroup";
  ...
  const iocConfirmForward = async () => {
    const selected = document
      .getElementById("iocFwdList")
      ?.querySelector(".ioc-fwd-item.selected");
    ...
    try {
      ...
      showToast("Lỗi chuyển tiếp!", "error");
    }
  };
  ...
  const iocSetupEmojiPicker = () => {
    ...
    picker.innerHTML = EMOJIS.map(
      (e) => `<button class="ioc-emoji-btn" data-e="${e}">${e}</button>`,
    ).join("");
  ...
  const iocRecall = async (msgId) => {
    ...
    try {
      await db
        .collection("messages")
        .doc(msgId)
        .update({ recalled: true, content: "" });
    } catch (e) {
      showToast("Lỗi thu hồi tin nhắn!", "error");
    }
  };
  ...
  const iocPin = async (msgId) => {
    ...
    try {
      ...
      showToast("Lỗi ghim tin nhắn!", "error");
    }
  };
  ...
  const iocConfirmEdit = async () => {
    ...
    try {
      ...
      showToast("Lỗi chỉnh sửa tin nhắn!", "error");
    }
  };
  ...
  // ── Subscription ───────────────────────────────────────────────────────────
  const setupCrmChat = () => {
    if (crmChatSubscription) {
      crmChatSubscription();
      crmChatSubscription = null;
    }
    _iocMsgs = [];
    iocRenderConvList();
    ...
    if (container)
      container.innerHTML =
        '<div class="crm-chat-loading">Đang tải tin nhắn...</div>';
    ...
    crmChatSubscription = db
      ...
      .onSnapshot(
        ...
        (err) => {
          ...
          if (c)
            c.innerHTML = `<div class="crm-chat-loading" style="color:#e53e3e">
  ...
  const iocBindEvents = () => {
    document
      .getElementById("btnSendCrmChat")
      ?.addEventListener("click", sendCrmChatMessage);
    document
      .getElementById("crmChatInput")
    ...
    document
      .getElementById("btnCancelIocReply")
      ?.addEventListener("click", iocCancelReply);

    document
      .getElementById("iocCtxMenu")
      ?.querySelectorAll(".ioc-ctx-item")
      .forEach((btn) => {
        btn.addEventListener("click", () => {
    ...
    document
      .getElementById("btnIocAttach")
      ?.addEventListener("click", () => fileInput?.click());
    fileInput?.addEventListener("change", () => {
      const file = fileInput.files[0];
      if (!file) return;
      if (file.size > 500 * 1024) {
        showToast("File quá lớn (tối đa 500KB)", "error");
        fileInput.value = "";
        return;
      }
    ...
    document
      .getElementById("btnClearFilePreview")
      ?.addEventListener("click", iocClearFilePreview);

    document
      .getElementById("btnIocAddPeople")
      ?.addEventListener("click", () => {
        const modal = document.getElementById("findFriendsModal");
        if (!modal) return;
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        if (
          !e.target.closest("#iocEmojiPicker") &&
          !e.target.closest("#btnIocEmoji")
        ) {
    ...
    document
      .getElementById("btnIocSearchMsg")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("btnCloseIocSearch")
      ?.addEventListener("click", () => {
        _iocSearchTerm = "";
        const bar = document.getElementById("iocMsgSearchBar");
    ...
    document
      .getElementById("iocMsgSearchInput")
    ...
    document
      .getElementById("crmChatSearch")
      ?.addEventListener("input", iocRenderConvList);

    document
      .getElementById("btnIocThreadBack")
      ?.addEventListener("click", () =>
        iocOpenThread(
          "group-global",
          "Nhóm Nội bộ Aladdin",
    ...
    document
      .getElementById("btnToggleIocInfo")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("btnIocShowMembers")
      ?.addEventListener("click", iocShowMembers);
    document
      .getElementById("btnIocShowMembersHdr")
      ?.addEventListener("click", () => {
        const panel = document.getElementById("iocInfoPanel");
        const layout = document.querySelector("#crm-chat-tab .ioc-layout");
        const btn = document.getElementById("btnToggleIocInfo");
    ...
    document
      .getElementById("btnIocBackFromMembers")
      ?.addEventListener("click", iocShowInfoMain);
    document
      .getElementById("btnIocShowPinned")
      ?.addEventListener("click", iocShowPinned);
    document
      .getElementById("btnIocBackFromPinned")
      ?.addEventListener("click", iocShowInfoMain);
    document
      .getElementById("btnIocBackFromMedia")
      ?.addEventListener("click", iocShowInfoMain);
    document
      .getElementById("btnIocShowImages")
      ?.addEventListener("click", () => iocShowMedia("image"));
    document
      .getElementById("btnIocShowFiles")
      ?.addEventListener("click", () => iocShowMedia("file"));

    // Favorites & notify
    document
      .getElementById("btnIocFavorite")
      ?.addEventListener("click", iocToggleFavorite);
    document
      .getElementById("btnIocNotify")
      ?.addEventListener("click", iocToggleNotify);

    // Group rename
    document
      .getElementById("btnIocRenameGroup")
      ?.addEventListener("click", iocRenameGroup);

    // Forward modal
    document
      .getElementById("btnCloseIocForward")
      ?.addEventListener("click", () => {
        document.getElementById("iocForwardModal").style.display = "none";
        _iocForwardId = null;
      });
    document
      .getElementById("btnCancelIocForward")
      ?.addEventListener("click", () => {
        document.getElementById("iocForwardModal").style.display = "none";
        _iocForwardId = null;
      });
    document
      .getElementById("btnConfirmIocForward")
      ?.addEventListener("click", iocConfirmForward);
    document
      .getElementById("iocFwdSearch")
      ?.addEventListener("input", (e) => iocRenderFwdList(e.target.value));

    // Filter row (visual-only for now — re-renders the list)
    document
      .getElementById("btnIocFilter")
      ?.addEventListener("click", () => iocRenderConvList());

    // New chat modal
    document
      .getElementById("btnIocNewChat")
      ?.addEventListener("click", iocOpenNewChat);
    document
      .getElementById("btnCloseIocNewChat")
      ?.addEventListener("click", () => {
        document.getElementById("iocNewChatModal").style.display = "none";
      });
    document
    ...
    document
      .getElementById("btnConfirmIocEdit")
      ?.addEventListener("click", iocConfirmEdit);
    document
      .getElementById("btnCancelIocEdit")
      ?.addEventListener("click", () => {
        document.getElementById("iocEditOverlay").style.display = "none";
        _iocEditingId = null;
      });
    document
      .getElementById("iocEditInput")
      ?.addEventListener("keydown", (e) => {
    ...
    document
      .getElementById("btnIocEmoji")
      ?.addEventListener("click", iocToggleEmoji);
  };
  ...
  const openCrmCustomerModal = async (customer) => {
    ...
    document.getElementById("crmModalTitle").textContent = isEdit
      ? "CHỈNH SỬA KHÁCH HÀNG"
      : "+ THÊM KHÁCH HÀNG MỚI";
    document.getElementById("btnSubmitCrmCustomer").textContent = isEdit
      ? "CẬP NHẬT KHÁCH HÀNG"
      : "LƯU KHÁCH HÀNG";
    ...
    document.getElementById("crmStatusSel").value =
  ...
  const setupCrmCustomerModal = () => {
    ...
    document
      .getElementById("btnCloseCrmCustomerModal")
      ?.addEventListener("click", closeModal);
    document
      .getElementById("btnCancelCrmCustomerModal")
      ?.addEventListener("click", closeModal);
    ...
    document
      .getElementById("btnSubmitCrmCustomer")
      ?.addEventListener("click", async () => {
  ...
  const openCrmOldCustomerModal = async (customer) => {
    ...
    if (!modal) return;
    const isEdit = !!customer;
    document.getElementById("crmOldModalTitle").textContent = isEdit
      ? "CHỈNH SỬA KHÁCH HÀNG CŨ"
      : "+ THÊM KHÁCH HÀNG CŨ";
    document.getElementById("btnSubmitCrmOldCustomer").textContent = isEdit
  ...
  const setupCrmOldCustomerModal = () => {
    ...
    document
      .getElementById("btnCloseCrmOldCustomerModal")
      ?.addEventListener("click", closeModal);
    document
      .getElementById("btnCancelCrmOldCustomerModal")
      ?.addEventListener("click", closeModal);
    modal.addEventListener("click", (e) => {
      if (e.target === modal) closeModal();
    });
    ...
    document
      .getElementById("btnSubmitCrmOldCustomer")
      ?.addEventListener("click", async () => {
  ...
  const initCrmModule = () => {
    if (!crmInitialized) {
      ...
      document
        .getElementById("btnBackToCrmList")
        ?.addEventListener("click", closeCrmProfile);
      ...
      document
        .getElementById("btnEditCrmCustomer")
        ?.addEventListener("click", async () => {
          if (!_currentCrmCustomer) return;
          closeCrmProfile();
          if (
            _currentCrmCustomer.status === "Đã xuất cảnh" ||
            _currentCrmCustomer.isCrmOldCustomer
          ) {
      ...
      document
        .getElementById("btnAddCrmCustomer")
        ?.addEventListener("click", () => {
          openCrmCustomerModal(null);
        });

      document
        .getElementById("btnAddCrmOldCustomer")
        ?.addEventListener("click", () => {
      ...
      document.getElementById("btnExportCrm")?.addEventListener("click", () => {
        ...
        const activeCustomers = _allCrmCustomers.filter(
          (c) => c.status !== "Đã xuất cảnh",
        );
        ...
        XLSX.writeFile(
          wb,
          `CRM_KhachHang_${new Date().toISOString().split("T")[0]}.xlsx`,
        );
        showToast("Đã xuất Excel thành công!", "success");
      });

      document
        .getElementById("btnExportCrmOld")
        ?.addEventListener("click", () => {
          if (!window.XLSX) {
            showToast("Thư viện Excel chưa sẵn sàng!", "warning");
            return;
          }
          const oldCustomers = _allCrmCustomers.filter(
            (c) => c.status === "Đã xuất cảnh" || c.isCrmOldCustomer,
          );
          ...
          XLSX.writeFile(
            wb,
            `CRM_KhachHangCu_${new Date().toISOString().split("T")[0]}.xlsx`,
          );
      ...
      document
        .getElementById("crmCountryFilter")
        ?.addEventListener("change", () => renderCrmCustomers(true));
      document
        .getElementById("crmStatusFilter")
        ?.addEventListener("change", () => renderCrmCustomers(true));

      document
        .getElementById("crmOldSearchInput")
        ?.addEventListener("input", () => renderCrmOldCustomers(true));
      document
        .getElementById("crmOldCountryFilter")
        ?.addEventListener("change", () => renderCrmOldCustomers(true));
      document
        .getElementById("crmOldStatusFilter")
        ?.addEventListener("change", () => renderCrmOldCustomers(true));

      document
        .getElementById("srcSearchInput")
        ?.addEventListener("input", () => renderCrmSource(true));
      document
        .getElementById("srcCountryFilter")
        ?.addEventListener("change", () => renderCrmSource(true));
      ...
      const openSourceModal = async () => {
        ...
        ["srcKetQua1", "srcKetQua2", "srcKetQua3", "srcKetQuaCuoi"].forEach(
          (id) => {
            const el = document.getElementById(id);
            if (el) el.value = "";
          },
      ...
      const closeSourceModal = () => {
        document.getElementById("sourceStudentModal").style.display = "none";
      };

      document
        .getElementById("btnAddSourceStudent")
        ?.addEventListener("click", openSourceModal);
      document
        .getElementById("btnCloseSourceModal")
        ?.addEventListener("click", closeSourceModal);
      document
        .getElementById("btnCancelSourceModal")
        ?.addEventListener("click", closeSourceModal);

      document
        .getElementById("btnSubmitSourceStudent")
        ?.addEventListener("click", async () => {
          ...
          const enrollDate =
            document.getElementById("srcEnrollDate")?.value || null;
          const payload = {
            name,
            status:
              document.getElementById("srcStatus")?.value ||
              "Chưa đi thi/chờ đơn",
            ...
            ngay_thi_cuoi:
              document.getElementById("srcNgayThiCuoi")?.value || null,
            ket_qua_cuoi:
              document.getElementById("srcKetQuaCuoi")?.value || null,
            updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
          };
          if (!editId) {
            payload.email = "";
          ...
          try {
            if (editId) {
              ...
              payload.code =
                "HNV" + String(_allCrmCustomers.length + 1).padStart(4, "0");
      ...
      document
        .getElementById("btnExportSourceExcel")
        ?.addEventListener("click", () => {
          if (!window.XLSX) {
      ...
      document
        .getElementById("crmStaffSearch")
        ?.addEventListener("input", () => renderCrmStaff(true));
      document
        .getElementById("crmStaffDeptFilter")
        ?.addEventListener("change", () => renderCrmStaff(true));
      document
        .getElementById("crmStaffStatusFilter")
        ?.addEventListener("change", () => renderCrmStaff(true));

      document
        .getElementById("btnCrmAddStaff")
        ?.addEventListener("click", () => {
          document.getElementById("hrmStaffEditId").value = "";
          document.getElementById("hrmStaffForm").reset();
          document.getElementById("hrmStaffModalTitle").textContent =
      ...
      document
        .getElementById("btnCloseCrmStaffProfile")
        ?.addEventListener("click", closeCrmStaffProfile);

      document
        .getElementById("btnCrmProfileEdit")
        ?.addEventListener("click", () => {
      ...
      if (currentUser) {
        ...
        if (av) {
          if (currentUser.avatar) {
            ...
            av.textContent = (currentUser.name || "U")
              .split(" ")
              .map((n) => n[0])
              .join("")
              .substring(0, 2)
    ...
    db.collection("students")
      ...
      .then((snap) => {
        ...
        snap.forEach((doc) => {
          const d = doc.data();
          d.id = doc.id;
          _allCrmCustomers.push(d);
        });
        renderCrmOverview();
        renderCrmCustomers();
        renderCrmOldCustomers();
        // Re-render source tab if it's visible
        if (document.getElementById("crm-source-tab")?.style.display !== "none")
          renderCrmSource();
      })
  ...
  const populateStaffProfileDashboard = (s) => {
    ...
    if (spSalaryEl) {
      spSalaryEl.textContent =
        s.salary > 0 ? Number(s.salary).toLocaleString("vi-VN") + " đ" : "-- đ";
      spSalaryEl.style.color =
        s.salary > 0 ? "#059669" : "var(--color-text-muted,#6B6A67)";
    }
    ...
    if (spWdEl) {
      spWdEl.textContent = "...";
      spWdEl.style.color = "var(--color-text-muted,#6B6A67)";
    }
    if (spAttEl) {
      spAttEl.textContent = "...";
      spAttEl.style.color = "var(--color-text-muted,#6B6A67)";
    }
    ...
    const _applySpHrScore = (kpiDisplay, attPct) => {
      ...
      if (scoreArcEl) {
        ...
        requestAnimationFrame(() => {
          scoreArcEl.style.transition =
            "stroke-dashoffset 1.1s cubic-bezier(.4,0,.2,1), stroke 0.3s";
          scoreArcEl.style.strokeDashoffset = String(
            326.7 - (totalScore / 100) * 326.7,
          );
        });
      }
      if (kpiBarEl) {
        ...
        requestAnimationFrame(() => {
          kpiBarEl.style.transition = "width 1s ease";
          kpiBarEl.style.width = kpiDisplay + "%";
        });
      }
      if (attBarEl) {
        attBarEl.style.width = "0%";
        requestAnimationFrame(() => {
          attBarEl.style.transition = "width 1s ease";
          attBarEl.style.width = attPct + "%";
        });
      }
      if (kpiValEl) kpiValEl.textContent = kpiDisplay + "%";
      if (attValEl) attValEl.textContent = attPct + "%";
      if (noteEl) {
        noteEl.textContent = gradeNote;
    ...
    setText(
      "spProfileContractStartDate",
      fmtDate(s.contractStartDate || s.joinDate),
    );
    setText(
      "spProfileContractEndDate",
      s.contractEndDate ? fmtDate(s.contractEndDate) : "Vô thời hạn",
    );
    setText(
      "spProfileContractStatus",
      s.status === "Đã nghỉ việc" ? "Hết hiệu lực" : "Đang hiệu lực",
    );
      ...
      spPhotoInput.addEventListener("change", (e) => {
        ...
        reader.onload = async (ev) => {
          try {
            ...
            if (spPhotoFrame)
              spPhotoFrame.innerHTML = `<img src="${thumbUrl}" alt="${esc(s.name || "")}">`;
            s.photoUrl = thumbUrl;
            await db
              .collection("hrm_staff")
              .doc(s.id)
            ...
            if (_hrmStorage) {
              try {
                ...
                if (hdBlob) {
                  ...
                  s.photoUrl = hdUrl;
                  if (spPhotoFrame)
                    spPhotoFrame.innerHTML = `<img src="${hdUrl}" alt="${esc(s.name || "")}">`;
                  showToast("Ảnh đã nâng cấp Full HD!", "success");
                }
              } catch (_) {
                /* HD upgrade failed — thumbnail already saved */
    ...
    if (spDocInput && !spDocInput.dataset.bound) {
      ...
      spDocInput.addEventListener("change", () =>
        uploadHrmResumeDocs(s.id, spDocInput, "sp"),
      );
    }
    ...
    if (spCDocInput && !spCDocInput.dataset.bound) {
      ...
      spCDocInput.addEventListener("change", () =>
        uploadHrmContractDocs(s.id, spCDocInput, "sp"),
      );
    }
  ...
  const initStaffProfileDashboard = async () => {
    ...
    const spWrapper = document.querySelector(
      "#staff-profile-dashboard .topbar-user-wrapper",
    );
    ...
    if (spWrapper && spDrop && !spWrapper.dataset.dropBound) {
      ...
      spWrapper
        .querySelector(".user-profile-badge-inline")
        ?.addEventListener("click", (e) => {
          e.stopPropagation();
          spDrop.style.display === "none" || !spDrop.style.display
            ? showDrop()
            : hideDrop();
        });

      // Close dropdown when any item inside is clicked (actual actions handled by global listeners)
      spDrop.addEventListener("click", () => {
        spDrop.style.display = "none";
      });
    ...
    const spTabNav = document.querySelector(
      "#staff-profile-dashboard .hrm-profile-topbar",
    );
    if (spTabNav && !spTabNav.dataset.spBound) {
      ...
      spTabNav.addEventListener("click", (e) => {
        ...
        document
          ...
          .forEach((t) => t.classList.remove("active"));
        document
          .querySelectorAll("#staff-profile-dashboard .crmsp-panel")
          .forEach((p) => p.classList.remove("active"));
        btn.classList.add("active");
      ...
      saveBtn.addEventListener("click", async () => {
        ...
        try {
          const snap2 = await db
            ...
            .get();
          if (snap2.empty) {
            showToast("Không tìm thấy hồ sơ nhân sự!", "error");
            return;
          }
    ...
    if (saveBankBtn && !saveBankBtn.dataset.bound) {
      ...
      saveBankBtn.addEventListener("click", async () => {
        ...
        saveBankBtn.innerHTML = "⏳ Đang lưu...";
        try {
          const snap2 = await db
            .collection("hrm_staff")
            .where("email", "==", currentUser.email)
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        document
          .getElementById("btnRequestLeave")
          ?.addEventListener("click", () => {
        ...
        if (avatarEl) {
          if (currentUser.avatar) {
            ...
            const initials = (currentUser.name || "NV")
              .split(" ")
              .map((w) => w[0])
              .join("")
              .slice(0, 2)
              .toUpperCase();
            avatarEl.textContent = initials;
            avatarEl.style.background = getAvatarBgColor(
              currentUser.name || "",
            );
          }
        }
        ...
        if (emailEl) {
          ...
          emailEl.href = currentUser.email
  ...
  const calcLeaveBalance = (joinDateRaw) => {
    ...
    const jd = joinDateRaw.toDate
      ? joinDateRaw.toDate()
      : typeof joinDateRaw === "string"
        ? new Date(joinDateRaw + "T00:00:00")
        : new Date(joinDateRaw);
    if (isNaN(jd)) return { total: 0, months: 0, joinStr: "--" };
    const months = Math.floor(
      (Date.now() - jd.getTime()) / (1000 * 60 * 60 * 24 * 30.44),
    );
  ...
  const renderLeavePanel = ({
    ...
    setEl(
      "LeaveMeta",
      `Tính từ ngày vào làm: ${joinStr} · ${months} tháng công tác`,
    );
  ...
  const setupLeaveModal = () => {
    ...
    document
      .getElementById("btnCloseLeaveModal")
    ...
    document
      .getElementById("btnConfirmLeave")
      ?.addEventListener("click", async () => {
  ...
  const saveFlightDate = async (
    ...
    await db
      ...
      .doc(studentId)
      .update({
        flightDate: flightTs || firebase.firestore.FieldValue.delete(),
      });

    if (flightTs) {
      await upsertFlightNotifications(
        studentId,
        studentName,
        studentCode,
        dateStrToMidnight(dateStr),
      );
    } else {
  ...
  const upsertFlightNotifications = async (
    studentId,
    studentName,
    studentCode,
    flightDate,
    ...
    await batch.commit();
    showToast(
      `Đã gửi thông báo lịch bay đến ${staffEmails.length} nhân viên.`,
      "success",
    );
  };
  ...
  const initNotificationBell = () => {
    ...
    [
      ...
      if (anchor.previousElementSibling?.classList.contains("topbar-notif-btn"))
        return;
      const btn = document.createElement("button");
    ...
    document
      ...
      ?.addEventListener("click", markAllNotifsRead);
  ...
  const renderNotifDropdown = () => {
    ...
    list.innerHTML = sorted
      .map((n) => {
        const dateObj =
          n.createdAt?.toDate?.() || n.dueDate?.toDate?.() || null;
        const timeStr = dateObj ? formatDateVN(dateObj) : "";
        const icon =
          n.type?.startsWith("flight") || n.dueDate
            ? "✈"
            : n.type === "task_assign"
              ? "📋"
              : "🔔";
  ...
  const initOrgChartDashboard = (() => {
    ...
    const renderOrgStaff = async (activeDept = "all") => {
      ...
      try {
        ...
        staff.sort(
          (a, b) =>
            (POSITION_RANK[a.jobTitle] ?? 9) -
              (POSITION_RANK[b.jobTitle] ?? 9) ||
        ...
        if (filterBar) {
          filterBar.innerHTML = depts
            ...
            .join("");
          filterBar
            .querySelectorAll(".oc-dept-btn")
            .forEach((btn) =>
              btn.addEventListener("click", () =>
                renderOrgStaff(btn.dataset.dept),
              ),
            );
        }
        ...
        if (!filtered.length) {
          grid.innerHTML =
            '<div class="oc-empty">Chưa có nhân sự trong mục này</div>';
          return;
        }
        ...
        grid.innerHTML =
          '<div class="oc-empty">Không thể tải dữ liệu nhân sự</div>';
      }
    };
  ...
  const gradeInfo = (score) => {
    if (score >= 9)
      return { grade: "Xuất sắc", color: "#6366F1", bg: "#EEF2FF" };
    if (score >= 7) return { grade: "Đạt", color: "#10B981", bg: "#ECFDF5" };
    if (score >= 5)
      return { grade: "Trung bình", color: "#F97316", bg: "#FFF7ED" };
    ...
    "Hành chính kế toán": [
      {
        ...
        opts: [
          ...
          "Bắt buộc dùng font Times New Roman",
          "Không cần yếu tố đặc biệt nào",
        ],
        ans: 0,
      },
      {
        ...
        opts: [
          "Ghi sổ hai lần để kiểm tra",
          "Mỗi giao dịch ghi Nợ và Có bằng nhau",
          "Lập báo cáo tài chính 2 lần/năm",
          "Sử dụng hai phần mềm kế toán song song",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          ...
          "Đăng ký vào sổ theo dõi và chuyển đúng bộ phận phụ trách",
          "Photo và lưu vào tủ ngay",
          "Chuyển thẳng cho giám đốc duyệt",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Chỉ cần tên người mua và số tiền",
          "Mã số thuế bên bán, bên mua, tên hàng hoá, ngày xuất hoá đơn",
          "Chữ ký giám đốc và con dấu doanh nghiệp",
          "Chỉ cần số hoá đơn và tổng tiền",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Chỉ cần Bảng cân đối kế toán",
          "Bảng cân đối kế toán, Kết quả kinh doanh, Lưu chuyển tiền tệ và Thuyết minh",
          "Bảng lương và sổ quỹ tiền mặt",
          "Báo cáo thuế và sổ cái",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Mua sắm tự do khi cần",
          "Kiểm kê định kỳ, bàn giao và thanh lý đúng quy trình",
          "Chỉ quan tâm khi tài sản hỏng",
          "Giao cho bảo vệ quản lý toàn bộ",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Dựa vào giá trị tiền tệ của tài sản",
          "Thời gian thu hồi hoặc sử dụng trong/ngoài 1 chu kỳ kinh doanh (thường 12 tháng)",
          "Dựa vào vị trí bảo quản tài sản",
          "Dựa vào tên gọi của loại tài sản",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Mua bổ sung khi nào hết",
          "Lập kế hoạch ngân sách và phê duyệt định mức theo tháng/quý",
          "Để nhân viên tự mua và thanh toán",
          "Mua số lượng lớn một lần để rẻ hơn",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          ...
          "Có agenda rõ ràng, đúng giờ và ghi biên bản đầy đủ",
          "Không cần chuẩn bị trước",
          "Họp ít nhất 2 tiếng để đủ nội dung",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Kỹ năng thiết kế đồ hoạ và lập trình",
          "Chính xác trong số liệu, tổ chức công việc và giao tiếp hiệu quả",
          "Kỹ năng thuyết trình trước đám đông",
          "Kỹ năng ngoại ngữ tiếng Anh lưu loát",
        ],
        ans: 1,
      },
    ],
    Marketing: [
      {
        ...
        opts: [
          "Số lượng nhân sự thực hiện chiến dịch",
          "CTR, CPC, ROAS và tỷ lệ chuyển đổi (Conversion Rate)",
          "Số bài đăng mạng xã hội mỗi tuần",
          "Số lần sản phẩm được nhắc tới trong báo chí",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Tốn kém hơn và khó đo lường hơn",
          "Cung cấp giá trị thực cho người dùng thay vì chỉ quảng bá sản phẩm",
          "Chỉ áp dụng trên các nền tảng số",
          "Không cần ngân sách để triển khai",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Sáng tạo → Sản xuất → Phân phối → Bán hàng",
          "Nhận thức → Quan tâm → Cân nhắc → Mua hàng → Giữ chân",
          "Nghiên cứu → Lên kế hoạch → Thực hiện → Đo lường",
          "Tiếp cận → Chuyển đổi → Tăng trưởng → Doanh thu",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Chạy quảng cáo trả phí trên Google",
          "Tối ưu nội dung và cấu trúc website để xuất hiện cao trên kết quả tìm kiếm tự nhiên",
          "Tăng số lượng followers trên mạng xã hội",
          "Thiết kế giao diện website đẹp hơn đối thủ",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "So sánh ngân sách hai chiến dịch khác nhau",
          "So sánh hai phiên bản nội dung/thiết kế để xác định phiên bản hiệu quả hơn",
          "Kiểm tra hiệu suất của hai nhân viên",
          "Thử nghiệm hai sản phẩm mới cùng lúc",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Ảnh chụp chân dung khách hàng thật",
          "Hồ sơ đại diện của khách hàng mục tiêu dựa trên dữ liệu nghiên cứu thực tế",
          "Danh sách tên các khách hàng đã mua hàng",
      ...
      },
      {
        ...
        opts: [
          "Chi phí trung bình mỗi lần nhấp chuột",
          "Chi phí trên mỗi 1.000 lượt hiển thị quảng cáo",
          "Chi phí mỗi lần người dùng điền form",
          "Chi phí mỗi khách hàng mua hàng thành công",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          ...
          "Tiêu đề hấp dẫn, nội dung đúng đối tượng và CTA rõ ràng",
          "Chỉ gửi vào các ngày cuối tuần",
          "Email càng dài càng thể hiện sự chuyên nghiệp",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Chi nhiều tiền để boosting nội dung",
          "Người dùng tự lan truyền nội dung dựa vào giá trị hoặc cảm xúc",
          "Hợp tác với người nổi tiếng quảng cáo",
          "Đăng nội dung nhiều lần để tăng tiếp cận",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Tổng doanh thu / Tổng chi phí vận hành",
          "(Doanh thu từ marketing − Chi phí marketing) / Chi phí marketing × 100%",
          "Số khách hàng mới / Tổng chi phí marketing",
          "Tổng lượt hiển thị / Chi phí quảng cáo",
        ],
        ans: 1,
      },
    ],
    "Đối ngoại": [
      {
        ...
        opts: [
          "Hợp đồng kinh tế có giá trị pháp lý cao nhất",
          "Biên bản ghi nhớ thể hiện ý định hợp tác giữa các bên",
          "Bản báo cáo tài chính hàng năm",
          "Hợp đồng lao động dành cho nhân sự nước ngoài",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Đặt nhà hàng và phòng nghỉ sang trọng",
          "Tìm hiểu văn hoá, giao thức ứng xử và mục đích chuyến thăm của đối tác",
          "Chuẩn bị quà tặng đắt tiền",
          "Mời toàn bộ nhân viên công ty tham gia đón tiếp",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Chờ đối tác liên hệ trước để tránh làm phiền",
          "Gửi email tóm tắt các điểm đã thống nhất trong vòng 24–48 giờ",
          "Lên lịch họp tiếp theo ít nhất 1 tháng sau",
          "Gọi điện ngay sau cuộc họp để xác nhận",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Kỹ năng thiết kế và trình bày bản vẽ",
          "Ngoại ngữ, lắng nghe chủ động và xây dựng quan hệ bền vững",
          "Kỹ năng tính toán số liệu tài chính",
      ...
      },
      {
        ...
        opts: [
          "Ném danh thiếp về phía đối tác để thể hiện sự thân thiện",
          "Nhận và trao danh thiếp bằng hai tay, xem qua trước khi cất cẩn thận",
          "Chỉ trao danh thiếp khi được yêu cầu",
          "Trao bằng một tay để tỏ ra tự tin",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Chỉ cần ghi tên công ty và số điện thoại liên hệ",
          "Giới thiệu tổ chức, mục tiêu hợp tác và giá trị mang lại cho cả hai bên",
          "Báo giá chi tiết tất cả dịch vụ ngay trong thư đầu tiên",
          "Liệt kê toàn bộ thành tích và giải thưởng của công ty",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Chỉ có ban giám đốc và cổ đông",
          "Đối tác, cơ quan nhà nước, truyền thông và cộng đồng địa phương",
          "Chỉ có khách hàng trực tiếp",
          "Chỉ bao gồm nhà cung cấp và đại lý phân phối",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Kiên quyết giữ lập trường để thể hiện sức mạnh",
          "Lắng nghe, tìm điểm chung và đề xuất giải pháp dựa trên lợi ích hai bên",
          "Tạm dừng hợp tác để tránh xung đột thêm",
          "Nhờ bên thứ ba làm trọng tài ngay lập tức",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Tài liệu giới thiệu công ty đã được ban hành",
          "Báo cáo nội bộ, chiến lược kinh doanh và thông tin tài chính chưa công bố",
          "Catalogue sản phẩm và bảng giá niêm yết",
          "Lịch sự kiện và chương trình khuyến mãi công khai",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Để ghi âm lại toàn bộ nội dung cuộc họp",
          "Đảm bảo thông điệp được truyền đạt chính xác, tránh hiểu nhầm dẫn đến tranh chấp",
          "Để giảm chi phí thuê phiên dịch ngoài",
          "Để tạo ấn tượng chuyên nghiệp với đối tác",
        ],
        ans: 1,
      },
    ],
    "Hồ sơ": [
      {
        ...
        opts: [
          "Chỉ cần hộ chiếu và ảnh thẻ",
          "Giấy tờ tuỳ thân, chứng minh tài chính, thư nhập học và lý lịch học tập",
          "Chỉ cần thư nhập học từ trường",
          "Chỉ cần hộ chiếu còn hạn và vé máy bay",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Tự điều chỉnh thông tin mà không cần báo học viên",
          "Thông báo ngay cho học viên và hướng dẫn bổ sung/điều chỉnh theo quy trình",
          "Nộp hồ sơ trước rồi điều chỉnh sau",
          "Huỷ hồ sơ và yêu cầu học viên làm lại từ đầu",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Hồ sơ đến trước xử lý trước (FIFO) bất kể hoàn cảnh",
          "Deadline nộp hồ sơ và mức độ khẩn cấp của từng trường hợp cụ thể",
          "Hồ sơ của học viên có nhiều dịch vụ hơn",
          "Xử lý hồ sơ đơn giản trước để giải quyết nhanh",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Để trên bàn làm việc để tiện tra cứu",
          "Lưu trữ trong tủ khoá, hạn chế người tiếp cận và sao lưu bản số hoá",
          "Gửi về nhà để bảo quản an toàn hơn",
          "Scan toàn bộ rồi huỷ bản gốc để tiết kiệm diện tích",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Bất kỳ độ phân giải nào cũng được",
          "Đủ sáng, không bị mờ/cắt góc, đúng định dạng và dung lượng file yêu cầu",
          "Chỉ cần chụp ảnh bằng điện thoại",
          "Chỉ cần nộp bản photo màu",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Để hồ sơ trên bàn đồng nghiệp và nhắn tin báo",
          "Lập biên bản bàn giao có chữ ký hai bên và ghi rõ danh sách tài liệu kèm theo",
          "Gửi ảnh chụp hồ sơ qua Zalo là đủ",
          "Giao trực tiếp không cần giấy tờ xác nhận",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Im lặng và nộp muộn mà không giải thích",
          "Thông báo ngay cho học viên và quản lý, đồng thời liên hệ phía tiếp nhận để xin gia hạn",
          "Đổ lỗi cho học viên chậm cung cấp giấy tờ",
          "Chờ thêm vài ngày rồi mới báo cáo",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Chỉ cần cập nhật khi hồ sơ được chấp nhận hoặc từ chối",
          "Thông báo mỗi khi có thay đổi quan trọng và ít nhất một lần mỗi tuần",
          "Để học viên tự hỏi khi cần thiết",
          "Cập nhật một lần duy nhất khi hoàn thành toàn bộ hồ sơ",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Thông báo thẳng cho học viên và đóng hồ sơ",
          "Phân tích nguyên nhân, tư vấn bổ sung/điều chỉnh và nộp lại đúng hướng dẫn",
          "Tự động chuyển sang trường/quốc gia khác",
          "Yêu cầu học viên đặt cọc thêm trước khi xử lý lại",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Để tránh đối thủ cạnh tranh biết học viên của mình",
          "Đây là dữ liệu cá nhân được bảo vệ theo quy định pháp luật về bảo mật thông tin",
          "Để học viên không tự nộp hồ sơ một mình",
          "Để tăng giá trị hồ sơ khi bàn giao cho trường",
        ],
        ans: 1,
      },
    ],
    "Đào tạo": [
      {
        ...
        opts: ["3 cấp độ", "4 cấp độ", "5 cấp độ", "6 cấp độ"],
        ans: 1,
      },
      {
        ...
        opts: [
          ...
          "Đào tạo trực tiếp ngay tại nơi làm việc",
          "Đào tạo theo nhóm lớn ngoài văn phòng",
          "Đào tạo từ xa qua video call",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Cụ thể, đo được, khả thi, liên quan và có thời hạn",
          "Đơn giản, nhanh chóng và tiết kiệm chi phí",
          "Sáng tạo, thú vị và hấp dẫn học viên",
          "Ngắn gọn, dễ nhớ và có thể chia sẻ rộng rãi",
        ],
        ans: 0,
      },
      {
        ...
        opts: [
          "Tương tác trực tiếp với giảng viên cao hơn",
          "Linh hoạt về thời gian và tiết kiệm chi phí đào tạo đáng kể",
          "Kiểm tra kết quả chặt chẽ hơn học offline",
          "Phù hợp hơn cho các kỹ năng thực hành phức tạp",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Đánh giá mức lương và phúc lợi nhân viên",
          "Xác định khoảng cách kỹ năng và nhu cầu đào tạo thực tế",
          "Lên lịch nghỉ phép cho nhân viên",
          "Phân tích ngân sách đào tạo hàng năm",
        ],
        ans: 1,
      },
      {
      ...
      },
      {
        ...
        opts: [
          "Tổng số giờ tham gia của học viên",
          "Mức độ áp dụng kiến thức vào công việc thực tế sau khoá học",
          "Số lượng tài liệu được phát cho học viên",
          "Mức độ hài lòng của giảng viên về buổi học",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Phương pháp học nhóm đông người",
          "Nhân viên mới được hỗ trợ bởi một nhân viên có kinh nghiệm",
          "Thi đua kết quả giữa các phòng ban",
          "Hệ thống kết đôi học viên để thi cùng nhau",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Hệ thống quản lý lương và phúc lợi nhân viên",
          "Nền tảng quản lý và triển khai các khoá đào tạo trực tuyến",
          "Phần mềm chấm công điện tử",
      ...
      },
      {
        ...
        opts: [
          "Lý thuyết và thực hành trong cùng một lớp học",
          "Học trực tuyến (online) và học trực tiếp (offline)",
          "Đào tạo cá nhân và đào tạo nhóm lớn",
          "Học nội bộ và học bên ngoài tổ chức",
        ],
        ans: 1,
      },
    ],
    "Kinh doanh": [
      {
        ...
        opts: [
          "Chốt giá ngay để không mất khách",
          "Xác định nhu cầu, mục tiêu và khả năng tài chính của khách hàng",
          "Giới thiệu tất cả sản phẩm dịch vụ hiện có",
          "Gửi báo giá chi tiết qua email trước",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Kỹ năng thiết kế tài liệu đẹp",
          "Lắng nghe, thấu hiểu nhu cầu và xây dựng niềm tin với khách hàng",
          "Nói nhanh và thuyết phục theo kịch bản",
          "Kỹ năng đàm phán giá và giảm chiết khấu",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          ...
          "Đề xuất khách hàng nâng cấp lên sản phẩm/gói dịch vụ cao hơn",
          "Bán thêm sản phẩm không liên quan",
          "Hoàn tiền cho khách hàng không hài lòng",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Giảm giá ngay lập tức để chốt nhanh",
          "Nhấn mạnh giá trị và lợi ích thực sự của dịch vụ so với chi phí bỏ ra",
          "Kết thúc buổi tư vấn và chờ khách quay lại",
          "Chuyển cho quản lý xử lý thay",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Chỉ cần tổng doanh thu đạt được",
          "Số lượng khách tiếp cận, tỷ lệ chốt hợp đồng và doanh thu thực tế so với target",
          "Danh sách tên khách hàng đã tư vấn",
          "Số giờ làm việc và số cuộc gọi thực hiện",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          ...
          "Quản lý thông tin và lịch sử tương tác với khách hàng để nâng cao chất lượng dịch vụ",
          "Thiết kế giao diện website bán hàng",
          "Tính toán hoa hồng và lương nhân viên kinh doanh",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Để thu thêm phí dịch vụ phát sinh",
          "Tạo sự tin tưởng, tăng khả năng giới thiệu khách mới và tái ký hợp đồng",
          "Để tránh khiếu nại và hoàn tiền",
          "Để đảm bảo khách hàng không chuyển sang đối thủ",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Chỉ khác nhau về độ tuổi và thu nhập",
          "Lead là người quan tâm chưa mua; Customer là người đã sử dụng dịch vụ",
          "Không có sự khác biệt trong quy trình chăm sóc",
          "Lead chỉ đến từ quảng cáo; Customer đến từ giới thiệu",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Tổng doanh thu / Tổng số lần liên hệ khách hàng",
          "Số hợp đồng ký được / Tổng số khách hàng tiếp cận × 100%",
          "Tổng số giờ tư vấn / Số hợp đồng",
          "Doanh thu tháng này / Doanh thu tháng trước × 100%",
        ],
        ans: 1,
      },
      {
        ...
        opts: [
          "Đổ lỗi cho thị trường và khách hàng khó tính",
          "Phân tích nguyên nhân, điều chỉnh cách tiếp cận và lên kế hoạch hành động cụ thể",
          "Tăng số lượng cuộc gọi lên gấp đôi ngay lập tức",
          "Xin quản lý hạ target xuống để dễ đạt hơn",
        ],
        ans: 1,
      },
    ],
  ...
  const loadCompetencyTestResults = () => {
    ...
    if (!document.getElementById("test-dashboard").dataset.tabsBound) {
      ...
      document.querySelectorAll(".test-admin-tab").forEach((tab) => {
        tab.addEventListener("click", () => {
          document
            .querySelectorAll(".test-admin-tab")
            .forEach((t) => t.classList.remove("active"));
          document.querySelectorAll(".test-admin-panel").forEach((p) => {
      ...
      document.querySelectorAll(".test-dept-filter").forEach((btn) => {
        btn.addEventListener("click", () => {
          document
            .querySelectorAll(".test-dept-filter")
            .forEach((b) => b.classList.remove("active"));
          btn.classList.add("active");
      ...
      document.querySelectorAll(".test-result-filter").forEach((btn) => {
        btn.addEventListener("click", () => {
          document
            .querySelectorAll(".test-result-filter")
            .forEach((b) => b.classList.remove("active"));
          btn.classList.add("active");
      ...
      document
        .getElementById("btnRefreshTestResults")
        ?.addEventListener("click", adminLoadResults);
      document
        .getElementById("btnCreateExam")
        ?.addEventListener("click", () => openExamCreateModal(null));
      document
        .getElementById("btnCloseExamModal")
        ?.addEventListener("click", () => {
          document.getElementById("examCreateModal").style.display = "none";
        });
      document
        .getElementById("btnCloseExamView")
        ?.addEventListener("click", () => {
          document.getElementById("examViewModal").style.display = "none";
        });
      document
        .getElementById("btnCloseTestDetail")
        ?.addEventListener("click", () => {
          document.getElementById("testDetailModal").style.display = "none";
        });
      document
        .getElementById("btnLoadTemplate")
        ?.addEventListener("click", loadExamTemplate);
      document
        .getElementById("examJsonFileInput")
        ?.addEventListener("change", importExamFile);
      document
        .getElementById("btnDownloadTemplate")
        ?.addEventListener("click", downloadExamTemplate);
      document
        .getElementById("btnSaveExamDraft")
        ?.addEventListener("click", () => saveExam(false));
      document
        .getElementById("btnSaveExamActive")
        ?.addEventListener("click", () => saveExam(true));
    ...
    if (examsTab) {
      examsTab.style.display = "flex";
      examsTab.classList.add("active-panel");
    }
    document
      .querySelectorAll(".test-admin-tab")
      .forEach((t) => t.classList.remove("active"));
    document
      .querySelector('.test-admin-tab[data-testtab="tab-exams"]')
  ...
  const adminLoadExams = async () => {
    ...
    if (!container) return;
    container.innerHTML =
      '<div style="padding:2rem;text-align:center;color:#6B6A67;">Đang tải…</div>';
    try {
      const snap = await db
        .collection("competency_exams")
        .orderBy("createdAt", "desc")
      ...
      container.innerHTML =
        '<div style="padding:2rem;text-align:center;color:#EF4444;">Không thể tải đề thi.</div>';
      return;
    }
    renderExamList();
  };
  ...
  const renderExamList = () => {
    ...
    const list =
      _examDeptFilter === "all"
        ? _examList
        : _examList.filter((e) => e.department === _examDeptFilter);
    if (!list.length) {
    ...
    container.innerHTML = list
      .map((ex) => {
        const dc = DEPT_COLORS[ex.department] || {
          color: "#6B6A67",
          bg: "#F5F4F2",
        };
        const date = ex.createdAt?.toDate
          ? ex.createdAt.toDate().toLocaleDateString("vi-VN")
          : "—";
    ...
    container.querySelectorAll(".exam-btn-view").forEach((btn) => {
      btn.addEventListener("click", () => {
        const ex = _examList.find((e) => e.id === btn.dataset.id);
        if (ex) openExamViewModal(ex);
      });
    });
    container.querySelectorAll(".exam-btn-edit").forEach((btn) => {
      btn.addEventListener("click", () => {
        const ex = _examList.find((e) => e.id === btn.dataset.id);
        if (ex) openExamCreateModal(ex);
      });
    });
    ...
    container.querySelectorAll(".exam-btn-delete").forEach((btn) => {
      btn.addEventListener("click", () => {
        if (confirm(`Xóa đề thi "${btn.dataset.title}"?`))
  ...
  const activateExam = async (examId) => {
    ...
    try {
      ...
      showToast(
        newState
          ? "Đề thi đã kích hoạt — nhân viên có thể thi ngay."
          : "Đề thi đã tắt.",
        "success",
      );
      adminLoadExams();
  ...
  const openExamCreateModal = (exam) => {
    ...
    document.getElementById("examModalTitle").textContent = exam
      ? "Sửa đề thi"
      : "Tạo đề thi mới";
    document.getElementById("examDeptSelect").value = exam
      ? exam.department || ""
      : "";
    document.getElementById("examTitleInput").value = exam
      ? exam.title || ""
      : "";
    ...
    const syncCountUI = () => {
      ...
      if (subtitle)
        subtitle.textContent = `Điền đầy đủ ${_examQCount} câu hỏi và đánh dấu đáp án đúng`;
    ...
    qContainer.onchange = (e) => {
      ...
      if (!block) return;
      block
        .querySelectorAll(".exam-q-opt-row")
        .forEach((row) => row.classList.remove("is-correct"));
  ...
  const loadExamTemplate = () => {
    ...
    if (!dept || !QUESTION_BANK[dept]) {
      showToast("Chọn phòng ban trước!", "error");
      return;
    }
  ...
  const applyImportedExam = (data, fileName) => {
    ...
    if (!qs || !Array.isArray(qs) || qs.length === 0) {
      showToast("Không tìm thấy câu hỏi trong file!", "error");
      return;
    }
    const validCounts = [10, 20, 30, 40, 100];
    if (!validCounts.includes(qs.length)) {
      showToast(
        `File có ${qs.length} câu — cần đúng 10, 20, 30, 40 hoặc 100 câu!`,
        "error",
      );
      return;
    }
    ...
    for (let i = 0; i < qs.length; i++) {
      ...
      if (!q.q || !q.opts || q.opts.length < 4 || q.ans === undefined) {
        showToast(
          `Câu ${i + 1} thiếu dữ liệu (câu hỏi / 4 đáp án / đáp án đúng)!`,
          "error",
        );
        return;
      }
    ...
    document.getElementById("examQuestionsContainer").innerHTML = qs
      .map((q, i) => buildQBlockHtml(i, q))
      .join("");
  ...
  const importExamFile = async (e) => {
    ...
    try {
      if (ext === "json") {
        ...
        showToast(
          "Định dạng không hỗ trợ! Dùng .xlsx, .docx, hoặc .json",
          "error",
        );
      }
    } catch (err) {
  ...
  const importExcel = async (file) => {
    ...
    if (rows.length < 4)
      throw new Error("File Excel cần ít nhất 4 dòng (xem template)");
    ...
    if (qRows.length < 10)
      throw new Error(
        `File chỉ có ${qRows.length} câu hỏi — cần ít nhất 10 câu`,
      );
    ...
    const questions = qRows.map((r, i) => {
      ...
      if (!q || !optA || !optB || !optC || !optD)
        throw new Error(
          `Dòng câu ${i + 1}: thiếu nội dung câu hỏi hoặc đáp án`,
        );
      if (ans === undefined)
        throw new Error(
          `Dòng câu ${i + 1}: đáp án đúng "${ansRaw}" không hợp lệ — nhập A, B, C hoặc D`,
        );
  ...
  const parseWordText = (text, fileName) => {
    ...
    for (const line of lines) {
      ...
      if (qMatch) {
        if (curQ && curQ.opts.length === 4 && curQ.ans !== undefined)
          questions.push(curQ);
      ...
      const ansMatch = line.match(
        /^[đd][aá]p\s*[aá]n(?:\s*[đd][uú]ng)?\s*[:：]\s*([ABCD])/i,
      );
      if (ansMatch && curQ) {
    ...
    if (questions.length === 0) {
      showToast(
        "Không đọc được câu hỏi. Kiểm tra lại format theo template!",
        "error",
      );
      return;
    }
  ...
  const downloadExamTemplate = () => {
    if (!window.XLSX) {
      showToast("Thư viện SheetJS chưa tải!", "error");
      return;
    }
    const dept =
      document.getElementById("examDeptSelect").value || DEPARTMENTS[0];
    ...
    const data = [
      ...
      ...Array.from({ length: _examQCount }, (_, i) => [
        `Câu hỏi ${i + 1}`,
        "Đáp án A",
        "Đáp án B",
        "Đáp án C",
    ...
    XLSX.writeFile(
      wb,
      `template-de-thi-${dept.toLowerCase().replace(/\s+/g, "-")}.xlsx`,
    );
  ...
  const collectExamFormData = () => {
    ...
    qContainer.querySelectorAll(".exam-q-builder").forEach((block, i) => {
      ...
      const opts = [...block.querySelectorAll(".exam-q-opt-input")].map((inp) =>
        inp.value.trim(),
      );
      ...
      opts.forEach((o, j) => {
        if (!o)
          errors.push(`Câu ${i + 1}: thiếu đáp án ${["A", "B", "C", "D"][j]}`);
      });
  ...
  const saveExam = async (activate) => {
    ...
    if (errors.length) {
      statusEl.textContent =
        "⚠ " +
        errors[0] +
        (errors.length > 1 ? ` (+${errors.length - 1} lỗi khác)` : "");
    ...
    try {
      ...
      if (activate) {
        ...
        const batch = db.batch();
        _examList
          .filter((e) => e.department === dept && e.id !== _examEditId)
          .forEach((e) => {
            batch.update(db.collection("competency_exams").doc(e.id), {
              isActive: false,
            });
          });
        await batch.commit();
      ...
      showToast(
        activate
          ? "Đề thi đã lưu và kích hoạt cho nhân viên!"
          : "Đã lưu đề thi (nháp).",
        "success",
      );
      adminLoadExams();
  ...
  const openExamViewModal = (exam) => {
    const dc = DEPT_COLORS[exam.department] || {
      color: "#6B6A67",
      bg: "#F5F4F2",
    };
    const date = exam.createdAt?.toDate
      ? exam.createdAt.toDate().toLocaleDateString("vi-VN")
      : "—";
    document.getElementById("examViewTitle").textContent =
      exam.title || "Xem đề thi";
    ...
    content.innerHTML = (exam.questions || [])
      .map(
        (q, i) => `
  ...
  const renderResultTable = () => {
    ...
    const avg = (
      list.reduce((s, r) => s + (r.score || 0), 0) / list.length
    ).toFixed(1);
    document.getElementById("testAvgScore").textContent = avg + "/10";
    document.getElementById("testPassCount").textContent = list.filter(
      (r) => r.score >= 7,
    ).length;
    document.getElementById("testFailCount").textContent = list.filter(
      (r) => r.score < 7,
    ).length;

    tbody.innerHTML = list
      .map((r) => {
        const dc = DEPT_COLORS[r.department] || {
          color: "#6B6A67",
          bg: "#F5F4F2",
        };
    ...
    tbody.querySelectorAll(".result-delete-btn").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        try {
          ...
          _testResultsAll = _testResultsAll.filter(
            (x) => x.id !== btn.dataset.rid,
          );
          renderResultTable();
  ...
  const initWorkflowDashboard = (() => {
    ...
    const STEP_SEEDS = {
      "Tuyển sinh du học Nhật Bản": {
        ...
        steps: [
          {
            name: "Tiếp nhận & tư vấn học viên",
            type: "task",
            assignee: "Tư vấn viên",
            description:
              "- Gặp gỡ, thu thập thông tin cá nhân học viên\n- Tư vấn chương trình phù hợp\n- Xác nhận mục tiêu và ngân sách",
            deadline: 3,
            checklist: [
              { text: "Thu thập thông tin cá nhân", done: false },
              { text: "Tư vấn chương trình", done: false },
            ],
            status: "done",
          },
          {
            name: "Chuẩn bị & kiểm tra hồ sơ",
            type: "task",
            assignee: "Chuyên viên hồ sơ",
            description:
              "- Hướng dẫn chuẩn bị giấy tờ\n- Kiểm tra tính hợp lệ\n- Dịch thuật và công chứng",
            deadline: 7,
            checklist: [
              { text: "Hộ chiếu còn hạn", done: true },
              { text: "Bằng tốt nghiệp/học bạ", done: true },
              { text: "Sổ hộ khẩu / CMND", done: false },
              { text: "Dịch thuật công chứng", done: false },
            ],
            status: "done",
          },
          {
            name: "Kiểm tra hồ sơ đạt yêu cầu?",
            type: "condition",
            assignee: "Lê Thị Mai",
            description: "Kiểm tra hồ sơ có đủ điều kiện nộp không",
            deadline: 2,
            checklist: [
              { text: "Kiểm tra tính hợp lệ hồ sơ", done: true },
              { text: "Đối chiếu giấy tờ", done: true },
              { text: "Kiểm tra điều kiện đầu vào", done: false },
              { text: "Cập nhật trạng thái hồ sơ", done: false },
            ],
            status: "active",
            conditionText: "Hồ sơ đạt yêu cầu",
            rejectStep: {
              name: "Yêu cầu bổ sung hồ sơ",
              assignee: "Lê Thị Mai",
              code: "03A",
            },
          },
          {
            name: "Ký hợp đồng & đóng học phí",
            type: "approve",
            assignee: "Kế toán",
            description:
              "- Ký hợp đồng dịch vụ\n- Thu học phí đợt 1\n- Cấp biên lai",
            deadline: 3,
            checklist: [
              { text: "Ký hợp đồng dịch vụ", done: false },
              { text: "Thu học phí đợt 1", done: false },
            ],
            status: "pending",
          },
          {
            name: "Xin visa Nhật Bản",
            type: "task",
            ...
            deadline: 21,
            checklist: [
              { text: "COE từ trường", done: false },
              { text: "Hồ sơ tài chính", done: false },
              { text: "Nhận visa", done: false },
            ],
            status: "pending",
          },
          {
            name: "Xuất cảnh sang Nhật Bản",
            type: "end",
            assignee: "Tư vấn viên",
            description: "Tiễn học viên và bàn giao đối tác Nhật",
            deadline: 1,
            checklist: [
              { text: "Mua vé máy bay", done: false },
              { text: "Xuất cảnh thành công", done: false },
            ],
            status: "pending",
          },
        ],
      ...
      "Tuyển sinh du học Hàn Quốc": {
        desc: "Quy trình xử lý hồ sơ tuyển sinh du học Hàn Quốc — D-2, D-4 visa",
        cat: "Tuyển sinh",
        steps: [
          {
            name: "Tư vấn & đánh giá năng lực",
            type: "task",
            assignee: "Tư vấn viên",
            description: "Đánh giá và tư vấn chương trình",
            deadline: 3,
            checklist: [{ text: "Đánh giá học vấn", done: false }],
            status: "pending",
          },
          {
            name: "Chuẩn bị hồ sơ",
            ...
            deadline: 10,
            checklist: [
              { text: "Hộ chiếu", done: false },
              { text: "Bằng tốt nghiệp", done: false },
            ],
            status: "pending",
          },
          {
            name: "Ký hợp đồng & thanh toán",
            type: "approve",
            assignee: "Kế toán",
            description: "Ký HĐ và thanh toán",
            deadline: 3,
            checklist: [{ text: "Ký hợp đồng", done: false }],
            status: "pending",
          },
          {
            name: "Nộp hồ sơ & nhận thư mời",
            type: "task",
            assignee: "Chuyên viên hồ sơ",
            description: "Gửi hồ sơ và nhận COR",
            deadline: 21,
            checklist: [
              { text: "Nộp hồ sơ", done: false },
              { text: "Nhận COR", done: false },
            ],
            status: "pending",
          },
          {
            name: "Xin visa D-2/D-4",
            type: "task",
            assignee: "Chuyên viên visa",
            description: "Nộp hồ sơ visa tại lãnh sự quán",
            deadline: 21,
            checklist: [
              { text: "Nộp visa", done: false },
              { text: "Nhận visa", done: false },
            ],
            status: "pending",
          },
          {
            name: "Xuất cảnh sang Hàn Quốc",
            type: "end",
            assignee: "Tư vấn viên",
            description: "Tiễn học viên xuất cảnh",
            deadline: 1,
            checklist: [{ text: "Mua vé máy bay", done: false }],
            status: "pending",
          },
        ],
      },
      "Xử lý gia hạn & thay đổi visa": {
        desc: "Hỗ trợ gia hạn visa, chuyển đổi visa",
        cat: "Visa",
        steps: [
          {
            ...
            type: "task",
            assignee: "Tư vấn viên",
            description: "Xác nhận yêu cầu gia hạn",
            deadline: 2,
            checklist: [{ text: "Kiểm tra visa hiện tại", done: false }],
            status: "pending",
          },
          {
            name: "Thu thập hồ sơ gia hạn",
            type: "task",
            assignee: "Chuyên viên visa",
            description: "Hộ chiếu, xác nhận đang học, tài chính",
            deadline: 5,
            checklist: [
              { text: "Hộ chiếu còn hạn", done: false },
              { text: "Xác nhận đang học", done: false },
            ],
            status: "pending",
          },
          {
            name: "Nộp hồ sơ gia hạn",
            type: "task",
            assignee: "Chuyên viên visa",
            description: "Nộp và theo dõi tiến độ",
            deadline: 7,
            checklist: [{ text: "Nộp hồ sơ", done: false }],
            status: "pending",
          },
          {
            name: "Nhận kết quả & bàn giao",
            type: "approve",
            assignee: "Tư vấn viên",
            description: "Nhận visa gia hạn và bàn giao",
            deadline: 3,
            checklist: [
              { text: "Nhận visa", done: false },
              { text: "Cập nhật hệ thống", done: false },
            ],
            status: "pending",
          },
        ],
      ...
      "Quy trình hoàn trả học phí": {
        desc: "Xử lý yêu cầu hoàn trả học phí",
        cat: "Tài chính",
        steps: [
          {
            name: "Tiếp nhận yêu cầu hoàn phí",
            type: "task",
            assignee: "Kế toán",
            description: "Xác nhận lý do và kiểm tra hợp đồng",
            deadline: 2,
            checklist: [{ text: "Kiểm tra hợp đồng", done: false }],
            status: "pending",
          },
          {
            ...
            type: "approve",
            assignee: "Giám đốc",
            description: "Xem xét và ký phê duyệt",
            deadline: 5,
            checklist: [
              { text: "Thẩm định hồ sơ", done: false },
              { text: "Ký phê duyệt", done: false },
            ],
            status: "pending",
          },
          {
            name: "Thực hiện hoàn trả",
            type: "task",
            assignee: "Kế toán",
            description: "Chuyển khoản và xác nhận",
            deadline: 5,
            checklist: [
              { text: "Chuyển khoản", done: false },
              { text: "Xác nhận nhận tiền", done: false },
            ],
            status: "pending",
          },
          {
            ...
            type: "end",
            assignee: "Kế toán",
            description: "Lưu hồ sơ và đóng ticket",
            deadline: 1,
            checklist: [{ text: "Cập nhật trạng thái", done: false }],
            status: "pending",
          },
        ],
      },
      "Quy trình thực tập sinh Nhật Bản": {
        desc: "Đưa thực tập sinh sang Nhật theo chương trình IM Japan",
        cat: "Xuất cảnh",
        steps: [
          {
            name: "Tuyển dụng & sơ tuyển",
            type: "task",
            assignee: "Tư vấn viên",
            description: "Tiếp nhận và sơ tuyển ứng viên",
            deadline: 7,
            checklist: [
              { text: "Kiểm tra độ tuổi", done: false },
              { text: "Sức khỏe cơ bản", done: false },
            ],
            status: "pending",
          },
          {
            ...
            type: "task",
            assignee: "Giáo viên",
            description: "Đào tạo N5-N4 và kỹ năng nghề",
            deadline: 90,
            checklist: [
              { text: "Hoàn thành tiếng Nhật", done: false },
              { text: "Đạt thi đầu ra N5", done: false },
            ],
            status: "pending",
          },
          {
            name: "Phỏng vấn với công ty Nhật",
            type: "approve",
            assignee: "Tư vấn viên",
            description: "Phỏng vấn và nhận kết quả",
            deadline: 14,
            checklist: [
              { text: "Tham dự phỏng vấn", done: false },
              { text: "Nhận kết quả", done: false },
            ],
            status: "pending",
          },
          {
            name: "Làm hồ sơ & xin visa",
            type: "task",
            assignee: "Chuyên viên hồ sơ",
            description: "Hồ sơ xuất cảnh và visa thực tập sinh",
            deadline: 30,
            checklist: [
              { text: "Hồ sơ visa TP", done: false },
              { text: "Khám sức khỏe", done: false },
            ],
            status: "pending",
          },
          {
            name: "Xuất cảnh sang Nhật",
            type: "end",
            assignee: "Tư vấn viên",
            description: "Tiễn và bàn giao đối tác Nhật",
            deadline: 1,
            checklist: [{ text: "Xuất cảnh thành công", done: false }],
            status: "pending",
          },
        ],
      },
    ...
    const TYPE_LABEL = {
      task: "Bước công việc",
      approve: "Bước phê duyệt",
      notify: "Bước thông báo",
      condition: "Bước điều kiện",
      parallel: "Bước song song",
      end: "Bước kết thúc",
    };
    ...
    const renderCanvas = (wf) => {
      ...
      startDiv.innerHTML =
        '<div class="wf2-node-start"><span>▶</span> BẮT ĐẦU</div>';
      ...
      if (steps.length) {
        ...
        drawArrow(
          svg,
          MX,
          SY + startDiv.offsetHeight,
          MX,
          nodeTop(0, extraByIdx[0]) - 2,
          firstColor,
        );
      }
      ...
      steps.forEach((s, i) => {
        ...
        if (s.type === "condition") {
          ...
          const txt = (s.conditionText || s.name || "?")
            .replace(/\?$/, "")
            .trim();
          wrap.innerHTML = `
          ...
          const dots = (s.checklist || [])
            .map(
              (c) =>
                `<span class="wf2-node-check-dot ${c.done ? "done" : ""}"></span>`,
            )
            .join("");
      ...
      if (steps.length) {
        ...
        const lastCardEl = nodes.querySelector(
          `[data-step-idx="${steps.length - 1}"]`,
        );
        const lastH =
          last.type === "condition" ? DW : (lastCardEl?.offsetHeight ?? NH);
        const lastColor =
          last.status === "done"
            ? last.type === "approve"
              ? "#F59E0B"
      ...
      endDiv.innerHTML =
        '<div class="wf2-node-end"><span>⏹</span> KẾT THÚC</div>';
      ...
      nodes.querySelectorAll("[data-del-idx]").forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          ...
          if (_activeStepIdx === null) {
            ...
            if (body)
              body.innerHTML = `<div class="wf2-empty-detail"><svg viewBox="0 0 24 24" style="width:36px;height:36px;fill:#CBD5E1"><path d="M14,2H6A2,2 0 0,0 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2M18,20H6V4H13V9H18V20Z"/></svg><p>Chọn một bước để xem chi tiết</p></div>`;
          }
          if (typeof showToast === "function")
            showToast(`Đã xóa "${stepName}"`, "info");
        });
      });
      ...
      const wrapEl =
        document.getElementById("wf2CanvasWrap") || canvasEl?.parentElement;
          ...
          const showDropLine = (dIdx) => {
            ...
            const lineY =
              dIdx < steps.length
                ...
                : nodeTop(steps.length - 1, extraByIdx[steps.length - 1]) +
                  NH +
                  GAP / 2;
          ...
          const onUp = async () => {
            ...
            if (!noOp) {
              ...
              if (_activeStepIdx === dragIdx) _activeStepIdx = insertAt;
                ...
                if (_activeStepIdx > dragIdx && _activeStepIdx <= insertAt)
                  _activeStepIdx--;
                else if (_activeStepIdx < dragIdx && _activeStepIdx >= insertAt)
                  _activeStepIdx++;
              }
              await saveStep(wf);
              renderCanvas(wf);
              renderSideList();
              if (typeof showToast === "function")
                showToast("Đã sắp xếp lại thứ tự bước", "success");
            }
          };
    ...
    const openStepDetail = (wf, idx) => {
        ...
        <div class="wf2-detail-field">
          ...
          <select id="wf2DStepType" class="wf2-detail-input wf2-detail-select">
            ${Object.entries(TYPE_LABEL)
              .map(
                ([v, l]) =>
                  `<option value="${v}" ${s.type === v ? "selected" : ""}>${TYPE_ICON[v]} ${l}</option>`,
              )
              .join("")}
          </select>
        </div>
      ...
      (async () => {
        ...
        html +=
          ...
          ROLE_OPTIONS.map(
            (r) =>
              `<option value="${r}"${r === curVal ? " selected" : ""}>${r}</option>`,
          ).join("") +
          "</optgroup>";
      ...
      if (_statusSel) {
        _statusSel.addEventListener("change", async function () {
          ...
          if (this.value === "done") {
            ...
            const nextIdx = wf.steps.findIndex(
              (st, i) => i > _activeStepIdx && st.status !== "done",
            );
            ...
            if (allDone) {
              ...
              if (typeof showToast === "function")
                showToast(
                  "🎉 Quy trình đã hoàn thành! Đã thêm vào danh sách.",
                  "success",
                );
              renderCanvas(wf);
              renderSideList();
            ...
            if (typeof showToast === "function")
              showToast(
                "✓ Bước hoàn thành! Chuyển sang bước tiếp theo.",
                "success",
              );
    ...
    const renderChecklist = (s, wf) => {
      ...
      cl.querySelectorAll(".wf2-check-inp").forEach((inp) => {
        inp.addEventListener("input", () => {
          s.checklist[+inp.dataset.ci].text = inp.value;
        });
      });
    ...
    const renderSideList = () => {
      const search = (
        document.getElementById("wf2SideSearch")?.value || ""
      ).toLowerCase();
      const cat = document.getElementById("wf2SideCat")?.value || "";
      const filtered = _workflows.filter(
        (w) =>
      ...
      body.innerHTML = paged
        .map((w) => {
          ...
          const upd =
            w.updatedAt?.toDate?.()?.toLocaleDateString("vi-VN") || "--";
      ...
      if (pgEl) {
        ...
        for (let p = 1; p <= pages; p++)
          pg += `<button class="wf2-page-btn ${p === _page ? "active" : ""}" data-p="${p}">${p}</button>`;
        pg += `<button class="wf2-page-btn" id="wf2PgNext">›</button>`;
        pgEl.innerHTML = pg;
        pgEl.querySelectorAll("[data-p]").forEach((btn) => {
          btn.addEventListener("click", () => {
            _page = +btn.dataset.p;
            renderSideList();
          });
        });
        document.getElementById("wf2PgPrev")?.addEventListener("click", () => {
          if (_page > 1) {
            _page--;
            renderSideList();
          }
        });
        document.getElementById("wf2PgNext")?.addEventListener("click", () => {
          if (_page < pages) {
            _page++;
            renderSideList();
          }
      ...
      body.querySelectorAll("[data-action]").forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          if (btn.dataset.action === "del") {
            if (confirm("Xóa quy trình này?")) deleteWorkflow(id);
    ...
    const renderListGrid = () => {
      const search = (
        document.getElementById("wf2ListSearch")?.value || ""
      ).toLowerCase();
    ...
    const openWorkflow = (id) => {
      ...
      if (actLbl)
        actLbl.textContent =
          wf.status === "active" ? "Đang hoạt động" : "Không hoạt động";
      ...
      const foot = document.getElementById("wf2DetailFoot");
      if (body)
        body.innerHTML = `<div class="wf2-empty-detail">
    ...
    const switchTab = (tab) => {
      ...
      document
        .querySelectorAll(".wf2-tab")
        .forEach((t) => t.classList.toggle("active", t.dataset.wftab === tab));
      if (panelList) panelList.style.display = tab === "list" ? "flex" : "none";
      if (panelEditor)
        panelEditor.style.display = tab === "editor" ? "flex" : "none";
    ...
    const saveWorkflow = async () => {
      ...
      if (id) {
        ...
        if (typeof showToast === "function")
          showToast(
            "Đã tạo quy trình mới! Kéo thả bước từ bảng bên trái để bắt đầu.",
            "success",
          );
        return;
      }
    ...
    return async () => {
      ...
      document.getElementById("wf2BtnNew")?.addEventListener("click", () => {
        ...
        _activeStepIdx = null;
        switchTab("editor");
        ["wf2FormName", "wf2FormDesc"].forEach((eid) => {
          const el = document.getElementById(eid);
          if (el) el.value = "";
        });
        const actEl = document.getElementById("wf2FormActive");
        ...
        if (svgEl2) {
          while (svgEl2.children.length > 1)
            svgEl2.removeChild(svgEl2.lastChild);
        }
      });

      // Save buttons
      document
        .getElementById("wf2BtnSaveWf")
        ?.addEventListener("click", saveWorkflow);
      document
        .getElementById("wf2BtnSaveFlow")
        ?.addEventListener("click", saveWorkflow);
      document
        .getElementById("wf2BtnCancel")
        ?.addEventListener("click", () => switchTab("list"));

      // Toggle active label
      document
        .getElementById("wf2FormActive")
      ...
      document.querySelectorAll(".wf2-palette-item").forEach((item) => {
        item.addEventListener("click", async () => {
          if (!_activeId) {
            if (typeof showToast === "function")
              showToast("Hãy lưu quy trình trước khi thêm bước!", "info");
            return;
          }
          const wf = _workflows.find((w) => w.id === _activeId);
          if (!wf) return;
          ...
          if (type === "condition") {
            ...
            newStep.rejectStep = {
              name: "Xử lý trường hợp không đạt",
              assignee: "",
              code: String(wf.steps.length + 1).padStart(2, "0") + "A",
            };
          }
          pushUndo(wf.steps);
          ...
          if (typeof showToast === "function")
            showToast(`Đã thêm "${newStep.name}"`, "success");
        });
      });

      // Update step
      document
        .getElementById("wf2BtnUpdateStep")
        ?.addEventListener("click", async () => {
          ...
          s.name =
            document.getElementById("wf2DStepName")?.value.trim() || s.name;
          s.type = document.getElementById("wf2DStepType")?.value || s.type;
          s.assignee =
            document.getElementById("wf2DStepAssignee")?.value.trim() || "";
          s.description =
            document.getElementById("wf2DStepDesc")?.value.trim() || "";
          s.deadline =
            parseInt(document.getElementById("wf2DStepDeadline")?.value) ||
            null;
          s.status =
            document.getElementById("wf2DStepStatus")?.value || s.status;
          if (s.type === "condition") {
            const ct = document
              .getElementById("wf2DConditionText")
              ?.value.trim();
            if (ct) s.conditionText = ct;
            if (!s.rejectStep) s.rejectStep = {};
            const rjName = document
              .getElementById("wf2DRejectName")
              ?.value.trim();
            const rjAssignee = document
              .getElementById("wf2DRejectAssignee")
              ?.value.trim();
            if (rjName) s.rejectStep.name = rjName;
            if (rjAssignee !== undefined) s.rejectStep.assignee = rjAssignee;
          }
          document
            .querySelectorAll("#wf2DChecklist .wf2-check-inp")
            .forEach((inp) => {
              const ci = +inp.dataset.ci;
              if (s.checklist && s.checklist[ci])
                s.checklist[ci].text = inp.value;
            });
          await saveStep(wf);
          if (typeof showToast === "function")
          ...
          if (allDoneNow) {
            ...
            if (typeof showToast === "function")
              showToast("🎉 Quy trình đã hoàn thành!", "success");
            renderSideList();
            renderListGrid();
          }
        });
      ...
      // Search/filter
      document
        .getElementById("wf2SideSearch")
        ?.addEventListener("input", renderSideList);
      document
        .getElementById("wf2SideCat")
        ?.addEventListener("change", renderSideList);
      document
        .getElementById("wf2ListSearch")
        ?.addEventListener("input", renderListGrid);
      document
        .getElementById("wf2ListCat")
        ?.addEventListener("change", renderListGrid);

      // ── Undo / Redo ─────────────────────────────────────────────────────
      syncUndoRedo();
      document
        .getElementById("wf2BtnUndo")
        ?.addEventListener("click", async () => {
          ...
          renderSideList();
          syncUndoRedo();
          if (_activeStepIdx !== null && wf.steps[_activeStepIdx])
            openStepDetail(wf, _activeStepIdx);
          if (typeof showToast === "function") showToast("Đã hoàn tác", "info");
        });
      document
        .getElementById("wf2BtnRedo")
        ?.addEventListener("click", async () => {
      ...
      // ── Preview ─────────────────────────────────────────────────────────
      document
        .getElementById("wf2BtnPreview")
        ?.addEventListener("click", () => {
          if (!_activeId) {
            showToast("Chưa có quy trình nào để xem trước", "info");
            return;
          }
          const wf = _workflows.find((w) => w.id === _activeId);
          if (!wf) return;
          ...
          if (!overlay) return;
          document.getElementById("wf2PreviewTitle").textContent =
            wf.name || "Xem trước quy trình";
          const stepCount = (wf.steps || []).length;
          const doneCount = (wf.steps || []).filter(
            (s) => s.status === "done",
          ).length;
          ...
          if (list) {
            list.innerHTML =
              ...
              <div class="wf2-preview-step-icon">${icon}</div>
                ...
                .join("") ||
              '<div style="text-align:center;padding:2rem;color:#94A3B8;">Chưa có bước nào</div>';
          }
          overlay.style.display = "flex";
        });
      document
        .getElementById("wf2BtnClosePreview")
        ?.addEventListener("click", () => {
          const o = document.getElementById("wf2PreviewOverlay");
          if (o) o.style.display = "none";
        });
      document
        .getElementById("wf2PreviewOverlay")
        ?.addEventListener("click", (e) => {
      ...
      // ── Settings ────────────────────────────────────────────────────────
      document
        .getElementById("wf2BtnSettings")
        ?.addEventListener("click", () => {
          if (!_activeId) {
            showToast("Chưa có quy trình nào để thiết lập", "info");
            return;
          }
          const wf = _workflows.find((w) => w.id === _activeId);
          if (!wf) return;
      ...
      document
        .getElementById("wf2BtnCloseSettings")
        ?.addEventListener("click", () => {
          const o = document.getElementById("wf2SettingsOverlay");
          if (o) o.style.display = "none";
        });
      document
        .getElementById("wf2SettingsOverlay")
        ?.addEventListener("click", (e) => {
          if (e.target === e.currentTarget)
            e.currentTarget.style.display = "none";
        });
      document
        .getElementById("wf2BtnSaveSettings")
        ?.addEventListener("click", async () => {
          if (!_activeId) return;
          const wf = _workflows.find((w) => w.id === _activeId);
          if (!wf) return;
      ...
      document
        .getElementById("wf2BtnCloseGuide")
        ?.addEventListener("click", () => {
          const o = document.getElementById("wf2GuideOverlay");
          if (o) o.style.display = "none";
        });
      document
        .getElementById("wf2GuideOverlay")
        ?.addEventListener("click", (e) => {
          if (e.target === e.currentTarget)
            e.currentTarget.style.display = "none";
        });
